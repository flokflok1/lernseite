...
- POST /api/v1/admin/ai/jobs - Create new AI job
- POST /api/v1/admin/ai/jobs/<id>/submit - Submit job for processing
...
# DDD Core Domain
from app.api.system_features.ai.core.factory import AIJobFactory
...
jobs_creation_bp = Blueprint(
    'ai_jobs_creation',
    __name__,
    url_prefix='/api/v1/admin/ai/jobs'
)
...
def create_ai_job() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # DDD: Use Factory to create job with business rules
        try:
            job_data = AIJobFactory.create_job(
