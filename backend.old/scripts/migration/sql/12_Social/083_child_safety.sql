...
CREATE INDEX idx_age_verif_user ON age_verifications(user_id);
CREATE INDEX idx_age_verif_status ON age_verifications(verification_status);
CREATE INDEX idx_age_verif_minor ON age_verifications(is_minor) WHERE is_minor = TRUE;
CREATE INDEX idx_age_verif_coppa ON age_verifications(is_coppa_protected) WHERE is_coppa_protected = TRUE;
-- Index for revalidation due dates (removed CURRENT_TIMESTAMP as it's not IMMUTABLE)
CREATE INDEX idx_age_verif_revalidation ON age_verifications(next_verification_due, verification_status)
    WHERE verification_status = 'verified';

COMMENT ON TABLE age_verifications IS 'Age verification system - COPPA & UK Age-Appropriate Design Code';
COMMENT ON COLUMN age_verifications.is_coppa_protected IS 'COPPA: Children under 13 (USA) - requires parental consent';
COMMENT ON COLUMN age_verifications.parental_consent_given IS 'COPPA: Verifiable parental consent required';
...
CREATE INDEX idx_parental_child ON parental_controls(child_user_id);
CREATE INDEX idx_parental_parent ON parental_controls(parent_user_id);
CREATE INDEX idx_parental_verified ON parental_controls(parent_verified);

COMMENT ON TABLE parental_controls IS 'Parental control settings - COPPA & UK Age-Appropriate Design Code';
COMMENT ON COLUMN parental_controls.allow_social_features IS 'COPPA: Social features disabled by default for under 13';
COMMENT ON COLUMN parental_controls.require_content_approval IS 'UK Code: Parent must approve child content';
...
CREATE INDEX idx_child_activity_child ON child_activity_log(child_user_id);
CREATE INDEX idx_child_activity_type ON child_activity_log(activity_type);
CREATE INDEX idx_child_activity_approval ON child_activity_log(requires_parent_approval, parent_approved)
    WHERE requires_parent_approval = TRUE;
CREATE INDEX idx_child_activity_created ON child_activity_log(created_at DESC);

COMMENT ON TABLE child_activity_log IS 'Child activity log for parent monitoring (privacy-protected)';
COMMENT ON COLUMN child_activity_log.visible_to_parent IS 'Some activities may be private (e.g., support messages)';
...
CREATE INDEX idx_screen_time_user ON screen_time_sessions(user_id);
CREATE INDEX idx_screen_time_date ON screen_time_sessions(user_id, session_date);
CREATE INDEX idx_screen_time_active ON screen_time_sessions(user_id, session_end) WHERE session_end IS NULL;

COMMENT ON TABLE screen_time_sessions IS 'Screen time tracking - UK Age-Appropriate Design Code';
COMMENT ON COLUMN screen_time_sessions.duration_minutes IS 'Session duration (auto-calculated on session_end)';
...
CREATE INDEX idx_daily_screen_user ON daily_screen_time(user_id);
CREATE INDEX idx_daily_screen_date ON daily_screen_time(summary_date DESC);
CREATE INDEX idx_daily_screen_exceeded ON daily_screen_time(limit_exceeded) WHERE limit_exceeded = TRUE;

COMMENT ON TABLE daily_screen_time IS 'Daily screen time summary with limits (UK Code)';

-- =====================================================
-- 6. GROOMING DETECTION LOGS (AI-Powered Safety)
-- =====================================================

CREATE TABLE IF NOT EXISTS grooming_detection_logs (
...
CREATE INDEX idx_grooming_groomer ON grooming_detection_logs(potential_groomer_id);
CREATE INDEX idx_grooming_victim ON grooming_detection_logs(potential_victim_id);
CREATE INDEX idx_grooming_risk ON grooming_detection_logs(risk_level);
CREATE INDEX idx_grooming_review ON grooming_detection_logs(human_review_required)
    WHERE human_review_required = TRUE;
CREATE INDEX idx_grooming_created ON grooming_detection_logs(created_at DESC);

COMMENT ON TABLE grooming_detection_logs IS 'AI-powered grooming detection (child safety critical)';
COMMENT ON COLUMN grooming_detection_logs.confidence_score IS 'AI confidence (threshold: 0.7 for flagging)';
COMMENT ON COLUMN grooming_detection_logs.escalated_to_authorities IS 'Legal requirement to report suspected child abuse';
...
CREATE TRIGGER trg_age_verification_calculate
    BEFORE INSERT OR UPDATE ON age_verifications
...
BEGIN
    IF NEW.session_end IS NOT NULL AND OLD.session_end IS NULL THEN
        ...
        INSERT INTO daily_screen_time (user_id, summary_date, total_minutes, session_count)
...
CREATE TRIGGER trg_screen_time_update_duration
    BEFORE UPDATE ON screen_time_sessions
