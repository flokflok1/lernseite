...
from app.repositories.subscription import SubscriptionPlansRepository
...
def list_pricing_plans() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Get plans
        plans = SubscriptionPlansRepository.get_all(
...
def get_pricing_plan(plan_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    """
    try:
        plan = SubscriptionPlansRepository.get_by_id(plan_id)
...
def update_pricing_plan(plan_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if data.get('active') is False and plan.get('active'):
            ...
            active_subscriptions = SubscriptionPlansRepository.get_active_subscription_count(plan_id)
        ...
        # Update plan
        updated_plan = SubscriptionPlansRepository.update(plan_id, data)
