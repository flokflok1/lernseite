...
from app.ai.configuration.prompt_models import PromptTemplate
...
def get_prompt_for_lm_id(lm_id: int) -> Optional[PromptTemplate]:
    ...
    Examples:
        ...
        if template:
            messages = template.render(context)
    """
    from app.ai.configuration.learning_method_mapping import get_prompt_key_for_lm_id
...
def get_prompts_by_group(group: str) -> List[PromptTemplate]:
    ...
    """
    from app.ai.configuration.learning_method_mapping import get_methods_by_group, LearningMethodGroup
