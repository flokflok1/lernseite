...
- GET    /api/v1/organisations/<id>/stats - Get organisation statistics
...
@api_v1.route('/organisations', methods=['GET'])
@admin_required
...
@api_v1.route('/organisations', methods=['POST'])
@admin_required
...
@api_v1.route('/organisations/<int:org_id>', methods=['GET'])
@token_required
...
@api_v1.route('/organisations/<int:org_id>', methods=['PUT'])
@token_required
...
@api_v1.route('/organisations/<int:org_id>/users', methods=['GET'])
@token_required
def list_organisation_users(org_id: int):
    ...
    try:
        ...
        if current_user['role'] not in ['admin', 'superadmin']:
            ...
            # Check org role (org_admin or teacher)
            from app.database.connection import fetch_one
            org_user_query = """
                SELECT org_role FROM organisation_users
                WHERE org_id = %s AND user_id = %s AND status = 'active'
            """
            org_user = fetch_one(org_user_query, (org_id, current_user['user_id']))

            if not org_user or org_user['org_role'] not in ['org_admin', 'teacher', 'trainer']:
                return jsonify({
                    'success': False,
                    'error': 'Forbidden',
                    'message': 'You do not have permission to view organisation users'
                }), 403
...
@api_v1.route('/organisations/<int:org_id>/assign-user', methods=['POST'])
@token_required
...
@api_v1.route('/organisations/<int:org_id>/stats', methods=['GET'])
@token_required
def get_organisation_stats(org_id: int):
    ...
    try:
        ...
        # Check permissions
        if current_user['role'] not in ['admin', 'superadmin']:
            # Check if user is org_admin of this organisation
            if current_user.get('organisation_id') != org_id:
                return jsonify({
                    'success': False,
                    'error': 'Forbidden',
                    'message': 'You do not have permission to view statistics of this organisation'
                }), 403
            ...
            if not org_user or org_user['org_role'] != 'org_admin':
                return jsonify({
                    'success': False,
                    'error': 'Forbidden',
