...
ISO 27001:2013 compliant - User profile and data management
"""
...
from app.services.audit_service import AuditService, EventType, EventCategory, Severity
...
@api_v1.route('/profile/change-password', methods=['POST'])
@token_required
def change_password():
    """
...
@api_v1.route('/profile', methods=['DELETE'])
@token_required
def delete_profile():
    """
...
@api_v1.route('/profile/subscription', methods=['GET'])
@token_required
def get_subscription():
    ...
    try:
        ...
        if subscription_data:
            ...
            response['expires_at'] = subscription_data['expires_at'].isoformat() if subscription_data.get('expires_at') else None
            response['auto_renew'] = subscription_data.get('auto_renew', False)
            response['billing_cycle'] = subscription_data.get('billing_cycle')
            response['started_at'] = subscription_data['started_at'].isoformat() if subscription_data.get('started_at') else None
        else:
...
@api_v1.route('/profile/tokens', methods=['GET'])
@token_required
def get_token_balance():
    ...
    try:
        ...
        token_balance = {
            ...
            'last_grant_date': wallet.get('last_grant_date').isoformat() if wallet.get('last_grant_date') else None,
            'source': source
        }
...
@api_v1.route('/profile/courses', methods=['GET'])
@token_required
...
@api_v1.route('/profile/activity', methods=['GET'])
@token_required
def get_activity():
    """
...
@api_v1.route('/profile/stats', methods=['GET'])
@token_required
...
@api_v1.route('/profile/theme', methods=['GET'])
@token_required
...
@api_v1.route('/profile/theme', methods=['PATCH'])
@token_required
...
@api_v1.route('/profile/preferences', methods=['GET'])
@token_required
...
@api_v1.route('/profile/preferences/window-sizes', methods=['GET'])
@token_required
...
@api_v1.route('/profile/preferences/window-sizes', methods=['PUT'])
@token_required
...
@api_v1.route('/profile/preferences/window-sizes/<window_type>', methods=['DELETE'])
@token_required
...
@api_v1.route('/profile/preferences/reset', methods=['POST'])
@token_required
