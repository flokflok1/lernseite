...
Consolidated admin course management with Factory Pattern integration.
...
def admin_create_course():
    ...
    Uses CourseFactory to ensure valid initial state with business rules.
    """
    try:
        ...
        # Use Factory Pattern to create course with valid initial state
        ...
        # Apply additional admin-specified fields
        admin_data = course_request.model_dump(exclude_none=True)
        for key, value in admin_data.items():
            if key not in ['title', 'category_id', 'description']:
...
def admin_update_course(course_id: str):
    ...
    try:
        ...
        AuditService.log_action(
            ...
            details={
                'updated_fields': list(
                    update_request.model_dump(exclude_none=True).keys()
                )
            },
            severity='medium'
        )
...
def admin_update_course_status(course_id: str):
    ...
    Uses CourseFactory for state transitions with business rule validation.
    """
    try:
        current_user = get_current_user()
        data = request.get_json()
        ...
        # Use Factory Pattern for state transitions
        result = None
        new_status = None

        if status_request.action == 'publish':
            try:
                published_course = CourseFactory.publish(
                    existing_course,
                    current_user['user_id']
                )
                result = CourseRepository.update(published_course)
            ...
            try:
                unpublished_course = CourseFactory.unpublish(
                    existing_course,
                    status_request.reason
                )
                result = CourseRepository.update(unpublished_course)
                new_status = 'draft'
            except ValueError as e:
                return jsonify({
                    'success': False,
                    'error': 'Cannot unpublish course',
                    'message': str(e)
                }), 400

        elif status_request.action == 'archive':
            archived_course = CourseFactory.archive(existing_course)
            result = CourseRepository.update(archived_course)
            new_status = 'archived'
