"""
Course Editor - Course Prompts Management

Feature-based course prompts management with permission-aware access.
Admin: Full access to all course prompts
User: Only access to prompts in own courses

Endpoints:
- GET    /api/v1/course-editor/manual/courses/{course_id}/prompts - List course prompts
- GET    /api/v1/course-editor/manual/courses/{course_id}/prompts/{scope} - Get prompt by scope
- PUT    /api/v1/course-editor/manual/courses/{course_id}/prompts/{scope} - Upsert prompt
- DELETE /api/v1/course-editor/manual/courses/{course_id}/prompts/{scope} - Delete prompt
- POST   /api/v1/course-editor/manual/courses/{course_id}/prompts/reset - Bulk reset prompts
- POST   /api/v1/course-editor/manual/courses/{course_id}/prompts/resolve - Resolve prompt
"""
...
def list_course_prompts(course_id: str):
...
@check_course_permission('read')
def get_course_prompt(course_id: str, scope: str):
...
def upsert_course_prompt(course_id: str, scope: str):
...
@check_course_permission('write')
def delete_course_prompt(course_id: str, scope: str):
...
@check_course_permission('write')
def bulk_reset_course_prompts(course_id: str):
...
@check_course_permission('read')
def resolve_course_prompt(course_id: str):
