"""
Course Editor - Manual CRUD Endpoints

Feature-based course editor with permission-aware access.
Admin: Full access to all courses
User: Only access to own courses

Endpoints:
- GET    /api/v1/course-editor/manual/courses - List courses (filtered by role)
- GET    /api/v1/course-editor/manual/courses/{id} - Get course details
...
from app.domain.models.admin.course import (
...
def list_courses():
    """
    List courses with permission-aware filtering.

    Admin: All courses
    User: Only own courses
    """
    try:
        ...
        # Permission-aware filtering: Users only see own courses
        if not is_admin:
            creator_id = user_id
...
@check_course_permission('write')
def update_course_status(course_id: str):
    """
    Change course status (publish, unpublish, archive, unarchive).

    Note: Publish/unpublish is admin-only.
    """
    try:
        current_user = get_current_user()
        data = request.get_json()
...
@check_course_permission('delete')
def permanent_delete_course(course_id: str):
    """
    Permanently delete a course (hard delete).

    WARNING: Cannot be undone! Admin-only operation.
    """
    try:
        current_user = get_current_user()
        data = request.get_json() or {}
