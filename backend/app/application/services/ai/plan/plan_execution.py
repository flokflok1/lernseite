        ...
        try:
            ...
            for phase_idx, phase in enumerate(plan_data.get('phases', [])):
                ...
                for step_idx, step in enumerate(phase.get('steps', [])):
                    ...
                    # Skip already completed/failed steps (resume support)
...
def _create_method_instances(
    ...
    # Deduplicate: allow max 1 method per group (A=0-4, B=5-8, C=9-11)
    lm_ids = _deduplicate_method_groups(lm_ids)
...
def _deduplicate_method_groups(lm_ids: list[int]) -> list[int]:
    """Keep only the first method per group (A=0-4, B=5-8, C=9-11).

    Prevents redundant methods like deep_explanation (0) + interactive_theory (2)
    which are both Group A (Explanatory) and produce nearly identical content.
    """
    ...
    for lm_id in lm_ids:
        ...
        if group not in seen_groups:
            ...
            logger.debug(f"Skipping duplicate group {group} method {lm_id}")
    return result
...
def _transform_data_for_method(
    ...
    # --- Type 2: Interactive Theory → {concept, examples[{code,explanation}], interactiveQuestion} ---
    if method_type == 2:
        # Ensure examples are objects {code, explanation}, not plain strings
        if raw_data.get('concept') or raw_data.get('examples'):
            examples = raw_data.get('examples', [])
            if examples and isinstance(examples[0], str):
                examples = [{'code': '', 'explanation': ex} for ex in examples]
            raw_data['examples'] = examples
            return raw_data
        # AI sometimes generates {questions: [{question, keywords}]} — convert
        if raw_data.get('questions') and isinstance(raw_data['questions'], list):
            qs = raw_data['questions']
            return {
                'concept': raw_text or qs[0].get('question', ''),
                'interactiveQuestion': qs[0].get('question', '') if qs else '',
                'examples': [{'code': '', 'explanation': q.get('question', '')} for q in qs[1:3]],
            }
    ...
    if method_type == 8:
        if raw_data.get('blanks') and raw_data.get('codeTemplate'):
            return raw_data
        blanks, template = _extract_cloze_with_template(raw_text)
        return {'blanks': blanks, 'codeTemplate': template}

    # --- Type 9: Free Text → {question, hints} ---
    if method_type == 9:
        if raw_data.get('question') and isinstance(raw_data['question'], str):
            return raw_data
        # AI sometimes generates {questions: [{question, keywords}]} — convert
        if raw_data.get('questions') and isinstance(raw_data['questions'], list):
            qs = raw_data['questions']
            return {
                'question': qs[0].get('question', raw_text) if qs else raw_text,
                'hints': qs[0].get('keywords', [])[:3] if qs else [],
            }
...
def _extract_cloze_with_template(text: str) -> tuple:
    """Create cloze blanks + codeTemplate from markdown bold terms.

    Returns (blanks, codeTemplate) where:
    - blanks: list of {position, answer, hint}
    - codeTemplate: text with **bold** terms replaced by ___
    """
    import re
    blanks: list[dict] = []
    # Strip markdown headings for cleaner template
    clean = re.sub(r'^#+\s+[^\n]+\n*', '', text, count=1).strip()
    # Collect bold terms as blanks (max 8)
    for m in re.finditer(r'\*\*(.+?)\*\*', clean):
        ...
        if 2 < len(term) < 50:
            blanks.append({
                'position': len(blanks),
                'answer': term,
                'hint': f'{term[0]}{"." * min(len(term) - 1, 3)}'
            })
            if len(blanks) >= 8:
                break
    # Build template: replace matched bold terms with ___
    template = clean
    replaced = 0
    for b in blanks:
        # Replace only first occurrence to maintain position order
        template = template.replace(f'**{b["answer"]}**', '___', 1)
        replaced += 1
...
def _skill_to_lesson_type(skill_code: str) -> str:
    ...
    mapping = {
        ...
        'generate_drag_and_drop': 'text',
        'generate_cloze_test': 'text',
        'generate_true_false': 'text',
        'generate_quiz': 'text',
        'generate_free_text': 'text',
        'generate_ihk_tasks': 'text',
        'generate_multi_step': 'text',
        'generate_math_interactive': 'text',
        'generate_comprehension_check': 'text',
        'generate_chapter_exam': 'text',
        'generate_timed_challenge': 'text',
