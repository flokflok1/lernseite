"""
LernsystemX AI Studio WebSocket Events

Real-time event handlers for KI-Authoring-Studio:
...
class AIStudioNamespace(Namespace):
    """
    AI Studio WebSocket Namespace

    Handles all real-time events for KI-Authoring-Studio sessions.
    Uses rooms for session-specific updates.
    """

    def __init__(self, namespace='/ai-studio'):
    ...
    def on_connect(self):
        ...
        try:
            ...
            if not token:
                logger.warning('AI Studio WebSocket connection without token')
                return False
            ...
            if not user_id:
                logger.warning('AI Studio WebSocket: Invalid token - no user_id')
                return False

            # Store user mapping
            self.connected_users[request.sid] = user_id
            logger.info(f'AI Studio WebSocket connected: user={user_id}, sid={request.sid}')

            emit('connected', {
                'status': 'connected',
                'user_id': user_id,
                'message': 'Connected to AI Studio'
            })

            return True

        except Exception as e:
            logger.error(f'AI Studio WebSocket connection error: {str(e)}')
            return False
    ...
    def on_disconnect(self):
        ...
        logger.info(f'AI Studio WebSocket disconnected: user={user_id}, sid={sid}')
...
def register_ai_studio_events(socketio):
    """
    Register AI Studio namespace and events

    Args:
        socketio: Flask-SocketIO instance
    """
    socketio.on_namespace(AIStudioNamespace('/ai-studio'))
    logger.info('AI Studio WebSocket namespace registered: /ai-studio')
...
def emit_generation_progress(socketio, session_id: str, progress: int, step: str, message: str, data: dict = None):
    ...
    socketio.emit('generation_progress', event_data, room=room, namespace='/ai-studio')
...
def emit_generation_complete(socketio, session_id: str, step: str, result: dict):
    ...
    socketio.emit('generation_complete', event_data, room=room, namespace='/ai-studio')
...
def emit_generation_error(socketio, session_id: str, step: str, error: str, details: dict = None):
    ...
    socketio.emit('generation_error', event_data, room=room, namespace='/ai-studio')
...
def emit_session_status_changed(socketio, session_id: str, old_status: str, new_status: str):
    ...
    socketio.emit('session_status', event_data, room=room, namespace='/ai-studio')
...
def emit_variant_generated(socketio, session_id: str, variant_type: str, variant_index: int, variant_id: str):
    ...
    socketio.emit('variant_generated', event_data, room=room, namespace='/ai-studio')
