...
CREATE INDEX IF NOT EXISTS idx_ula_user ON public.user_learning_analytics(user_id);
CREATE INDEX IF NOT EXISTS idx_ula_course ON public.user_learning_analytics(course_id);
CREATE INDEX IF NOT EXISTS idx_ula_topic ON public.user_learning_analytics(topic);
CREATE INDEX IF NOT EXISTS idx_ula_score ON public.user_learning_analytics(score_avg);
CREATE INDEX IF NOT EXISTS idx_ula_user_course ON public.user_learning_analytics(user_id, course_id);

-- Trigger für updated_at
DROP TRIGGER IF EXISTS ula_updated ON public.user_learning_analytics;
...
COMMENT ON TABLE public.user_learning_analytics IS 'Lernfortschritt und Leistungsdaten pro Thema für adaptive Prüfungsvorbereitung';
