...
-- Stores configuration/settings schema for 25 System-Features
...
-- Strategy: Configuration schemas with i18n support
-- ============================================================================

-- Whiteboard Engine - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  "feature_code": "whiteboard_engine",
  "settings_type": "infrastructure",
  "fields": [
    {
      "name": "enable_formula_recognition",
      "type": "checkbox",
      "label_fallback": "Enable Formula Recognition",
      "default": true
    },
    {
      "name": "enable_diagram_detection",
      "type": "checkbox",
      "label_fallback": "Enable Diagram Detection",
      "default": true
    },
    {
      "name": "max_file_size_mb",
      "type": "number",
      "label_fallback": "Max File Size (MB)",
      "default": 50
    }
  ]
}'::jsonb WHERE feature_code = 'whiteboard_engine';

-- IT Sandbox - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  "feature_code": "it_sandbox",
  "settings_type": "infrastructure",
  "fields": [
    {
      "name": "sandbox_timeout_minutes",
      "type": "number",
      "label_fallback": "Sandbox Timeout (minutes)",
      "default": 30
    },
    {
      "name": "enable_code_execution",
      "type": "checkbox",
      "label_fallback": "Allow Code Execution",
      "default": true
    },
    {
      "name": "enable_network_access",
      "type": "checkbox",
      "label_fallback": "Allow Network Access",
      "default": false
    }
  ]
}'::jsonb WHERE feature_code = 'it_sandbox';

-- IHK Exam System - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  "feature_code": "ihk_exam_system",
  "settings_type": "assessment",
  "fields": [
    {
      "name": "exam_duration_minutes",
      "type": "number",
      "label_fallback": "Exam Duration (minutes)",
      "default": 180
    },
    {
      "name": "passing_score_percent",
      "type": "number",
      "label_fallback": "Passing Score (%)",
      "default": 60
    }
  ]
}'::jsonb WHERE feature_code = 'ihk_exam_system';

-- NPC Tutor - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  ...
  "fields": [
    {
      "name": "tutor_persona",
      "type": "select",
      "label_fallback": "Tutor Persona",
      "options": ["friendly_mentor", "strict_professor", "socratic_guide"],
      "default": "friendly_mentor"
    },
    {
      "name": "response_language",
      "type": "select",
      "label_fallback": "Response Language",
      "options": ["de", "en", "pl"],
      "default": "de"
    }
  ]
}'::jsonb WHERE feature_code = 'npc_tutor';

-- XP & Quest System - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  "feature_code": "xp_quest_system",
  "settings_type": "gamification",
  "fields": [
    {
      "name": "xp_per_lesson",
      "type": "number",
      "label_fallback": "XP per Lesson",
      "default": 10
    },
    {
      "name": "level_threshold",
      "type": "number",
      "label_fallback": "Level Up Threshold (XP)",
      "default": 100
    }
  ]
}'::jsonb WHERE feature_code = 'xp_quest_system';

-- Daily Recall (Spaced Repetition) - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  "feature_code": "daily_recall",
  "settings_type": "gamification",
  "fields": [
    {
      "name": "min_hours_between_recalls",
      "type": "number",
      "label_fallback": "Min Hours Between Recalls",
      "default": 12
    },
    {
      "name": "max_daily_recalls",
      "type": "number",
      "label_fallback": "Max Daily Recalls",
      "default": 10
    }
  ]
}'::jsonb WHERE feature_code = 'daily_recall';

-- Speech-to-Text Engine - Feature configuration
UPDATE support_systems.system_features SET ui_schema = '{
  ...
  "fields": [
    {
      "name": "language",
      "type": "select",
      "label_fallback": "Recognition Language",
      "options": ["de", "en", "pl"],
      "default": "de"
    },
    {
      ...
      "type": "checkbox",
      "label_fallback": "Enable Transcription",
      "default": true
    }
  ]
}'::jsonb WHERE feature_code = 'speech_to_text';
...
COMMENT ON COLUMN support_systems.system_features.ui_schema IS
'JSONB schema for feature configuration settings. Used for dynamic settings UI generation.
Separate from Content-LM schemas. Includes i18n keys + English fallbacks.
Only key features populated; others can be added as needed.';
