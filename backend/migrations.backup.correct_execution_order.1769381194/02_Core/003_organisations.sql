...
-- ============================================================================
CREATE TABLE IF NOT EXISTS organisations.organization_members (
...
CREATE INDEX IF NOT EXISTS idx_org_members_org ON organisations.organization_members (organization_id, status);
CREATE INDEX IF NOT EXISTS idx_org_members_user ON organisations.organization_members (user_id);
CREATE INDEX IF NOT EXISTS idx_org_members_role ON organisations.organization_members (role_in_org);
CREATE INDEX IF NOT EXISTS idx_org_members_status ON organisations.organization_members (status) WHERE status = 'active';

COMMENT ON TABLE organisations.organization_members IS 'Membership relationship between users and organizations';

-- ============================================================================
-- TABLE: organization_classes
-- Description: Classes/groups within organizations
-- ============================================================================
CREATE TABLE IF NOT EXISTS organisations.organization_classes (
...
CREATE INDEX IF NOT EXISTS idx_org_classes_org ON organisations.organization_classes (organization_id);
CREATE INDEX IF NOT EXISTS idx_org_classes_teacher ON organisations.organization_classes (teacher_id);
CREATE INDEX IF NOT EXISTS idx_org_classes_code ON organisations.organization_classes (class_code);
CREATE INDEX IF NOT EXISTS idx_org_classes_status ON organisations.organization_classes (status) WHERE status = 'active';

COMMENT ON TABLE organisations.organization_classes IS 'Classes/groups for schools and training programs';
...
CREATE TABLE IF NOT EXISTS courses.class_enrollments (
    ...
    class_id UUID REFERENCES organisations.organization_classes(class_id) ON DELETE CASCADE,
    user_id UUID REFERENCES core.users(user_id) ON DELETE CASCADE,
...
CREATE TRIGGER update_org_classes_updated_at BEFORE UPDATE ON organisations.organization_classes
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
