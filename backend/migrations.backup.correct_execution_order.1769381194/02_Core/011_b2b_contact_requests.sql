...
CREATE TABLE IF NOT EXISTS b2b_contact_requests (
    ...
    organisation_id UUID REFERENCES organisations.organisations(id),
    -- NULL until conversion, then links to created org for tracking

    -- Admin Assignment
    assigned_to UUID REFERENCES users(id),  -- Which admin is handling this request
...
CREATE INDEX idx_b2b_requests_org ON b2b_contact_requests(organisation_id)
    WHERE organisation_id IS NOT NULL;

-- Company name search (admin search functionality)
CREATE INDEX idx_b2b_requests_company ON b2b_contact_requests USING gin(to_tsvector('german', company_name));

-- Assigned admin queries (my requests view)
CREATE INDEX idx_b2b_requests_assigned ON b2b_contact_requests(assigned_to)
    WHERE assigned_to IS NOT NULL;

-- Composite index for admin panel filters (status + date)
CREATE INDEX idx_b2b_requests_status_date ON b2b_contact_requests(status, created_at DESC);
...
CREATE TRIGGER trg_b2b_request_update
    BEFORE UPDATE ON b2b_contact_requests
    FOR EACH ROW
...
COMMENT ON TABLE b2b_contact_requests IS
...
COMMENT ON COLUMN b2b_contact_requests.status IS
...
COMMENT ON COLUMN b2b_contact_requests.notes IS
...
COMMENT ON COLUMN b2b_contact_requests.proposed_plan IS
...
COMMENT ON COLUMN b2b_contact_requests.proposed_price IS
'Custom negotiated monthly price in EUR. NULL if standard pricing applies. Set during offer_sent stage.';

COMMENT ON COLUMN b2b_contact_requests.organisation_id IS
...
COMMENT ON COLUMN b2b_contact_requests.assigned_to IS
'Admin/sales rep handling this request. Used for workload distribution and accountability.';

COMMENT ON COLUMN b2b_contact_requests.metadata IS
...
SELECT
    'b2b_contact_requests table created successfully' AS status,
