...
CREATE INDEX IF NOT EXISTS idx_media_files_org ON billing_storage.media_files(organization_id);
...
CREATE TABLE IF NOT EXISTS storage.storage_packages (
    ...
    storage_limit_mb BIGINT NOT NULL,  -- NULL = unlimited
...
BEGIN
    ...
    INSERT INTO storage.quarantine_log (
        ...
        v_file.uploaded_by, v_file.organization_id
    );
...
CREATE POLICY tenant_isolation_policy ON billing_storage.media_files
    FOR ALL
    USING (
        organization_id IS NULL  -- System files
        OR organization_id = current_organisation_id()  -- Own org
