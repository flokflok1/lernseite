...
CREATE INDEX IF NOT EXISTS idx_analytics_sessions_org ON public.analytics_sessions(organization_id, started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_analytics_agg_metric ON public.analytics_aggregates(metric_type, date DESC);
CREATE INDEX IF NOT EXISTS idx_analytics_agg_dimension ON public.analytics_aggregates(dimension, dimension_value, date DESC);
CREATE INDEX IF NOT EXISTS idx_analytics_agg_date ON public.analytics_aggregates(date DESC, hour);

COMMENT ON TABLE public.analytics_aggregates IS 'Pre-aggregated analytics for dashboards (daily/hourly)';
