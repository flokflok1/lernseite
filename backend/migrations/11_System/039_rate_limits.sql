...
CREATE INDEX IF NOT EXISTS idx_rate_limits_org ON core.rate_limits(organisation_id);
...
CREATE INDEX IF NOT EXISTS idx_rate_limit_hits_org ON core.rate_limit_hits(organisation_id, hit_at DESC);
...
CREATE TABLE IF NOT EXISTS billing_storage.quota_usage (
    ...
    UNIQUE (organisation_id, resource_type, period_start)
);

CREATE INDEX IF NOT EXISTS idx_quota_usage_user ON billing_storage.quota_usage(user_id, resource_type);
CREATE INDEX IF NOT EXISTS idx_quota_usage_org ON billing_storage.quota_usage(organisation_id, resource_type);
