...
CREATE TABLE IF NOT EXISTS support_systems.feature_flag_role_mappings (
...
CREATE INDEX idx_feature_role_feature ON support_systems.feature_flag_role_mappings(feature_name);
CREATE INDEX idx_feature_role_role ON support_systems.feature_flag_role_mappings(role_code);
CREATE INDEX idx_feature_role_enabled ON support_systems.feature_flag_role_mappings(is_enabled);
CREATE INDEX idx_feature_role_priority ON support_systems.feature_flag_role_mappings(priority_level);

COMMENT ON TABLE support_systems.feature_flag_role_mappings IS 'Role-based access control with per-role quotas';
COMMENT ON COLUMN support_systems.feature_flag_role_mappings.max_usage_per_day IS 'Daily limit (NULL = unlimited)';
COMMENT ON COLUMN support_systems.feature_flag_role_mappings.priority_level IS '0=normal, 10=premium, 20=enterprise';

-- =====================================================
-- 2. FEATURE TIER LIMITS (Subscription tier-based quotas)
-- =====================================================

CREATE TABLE IF NOT EXISTS support_systems.feature_flag_tier_limits (
    id SERIAL PRIMARY KEY,
    feature_name VARCHAR(100) NOT NULL REFERENCES support_systems.feature_flags(name) ON DELETE CASCADE,
...
CREATE INDEX idx_feature_tier_feature ON support_systems.feature_flag_tier_limits(feature_name);
CREATE INDEX idx_feature_tier_tier ON support_systems.feature_flag_tier_limits(tier_code);
CREATE INDEX idx_feature_tier_enabled ON support_systems.feature_flag_tier_limits(is_enabled);

COMMENT ON TABLE support_systems.feature_flag_tier_limits IS 'Tier-based feature limits (free, premium, enterprise)';

-- =====================================================
-- 3. FEATURE ROLLOUT PLANS (4-Phase progressive rollout)
-- =====================================================

CREATE TABLE IF NOT EXISTS support_systems.feature_flag_rollout_plans (
    id SERIAL PRIMARY KEY,
    feature_name VARCHAR(100) NOT NULL REFERENCES support_systems.feature_flags(name) ON DELETE CASCADE,
...
CREATE INDEX idx_rollout_feature ON support_systems.feature_flag_rollout_plans(feature_name);
CREATE INDEX idx_rollout_status ON support_systems.feature_flag_rollout_plans(status);
CREATE INDEX idx_rollout_active ON support_systems.feature_flag_rollout_plans(status) WHERE status = 'active';

COMMENT ON TABLE support_systems.feature_flag_rollout_plans IS '4-phase progressive rollout (canary → early → wider → full)';

-- =====================================================
-- 4. FEATURE A/B TESTS (Variant testing)
-- =====================================================

CREATE TABLE IF NOT EXISTS support_systems.feature_flag_ab_tests (
    id SERIAL PRIMARY KEY,
    feature_name VARCHAR(100) NOT NULL REFERENCES support_systems.feature_flags(name) ON DELETE CASCADE,
...
CREATE INDEX idx_ab_feature ON support_systems.feature_flag_ab_tests(feature_name);
CREATE INDEX idx_ab_status ON support_systems.feature_flag_ab_tests(status);
CREATE INDEX idx_ab_active ON support_systems.feature_flag_ab_tests(status) WHERE status = 'active';

COMMENT ON TABLE support_systems.feature_flag_ab_tests IS 'A/B test configuration for feature variants';
...
CREATE TRIGGER trg_feature_role_mappings_updated_at
    BEFORE UPDATE ON support_systems.feature_flag_role_mappings
    FOR EACH ROW EXECUTE FUNCTION update_feature_config_timestamp();

CREATE TRIGGER trg_feature_tier_limits_updated_at
    BEFORE UPDATE ON support_systems.feature_flag_tier_limits
    FOR EACH ROW EXECUTE FUNCTION update_feature_config_timestamp();

CREATE TRIGGER trg_feature_rollout_plans_updated_at
    BEFORE UPDATE ON support_systems.feature_flag_rollout_plans
    FOR EACH ROW EXECUTE FUNCTION update_feature_config_timestamp();

CREATE TRIGGER trg_feature_ab_tests_updated_at
    BEFORE UPDATE ON support_systems.feature_flag_ab_tests
    FOR EACH ROW EXECUTE FUNCTION update_feature_config_timestamp();
