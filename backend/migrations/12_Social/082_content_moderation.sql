...
CREATE TABLE IF NOT EXISTS moderation.content_reports (
...
CREATE INDEX idx_reports_content ON moderation.content_reports(reported_content_type, reported_content_id);
CREATE INDEX idx_reports_reporter ON moderation.content_reports(reporter_user_id);
CREATE INDEX idx_reports_status ON moderation.content_reports(status);
CREATE INDEX idx_reports_priority ON moderation.content_reports(priority);
CREATE INDEX idx_reports_category ON moderation.content_reports(report_category);
CREATE INDEX idx_reports_assigned ON moderation.content_reports(assigned_moderator_id);
CREATE INDEX idx_reports_sla ON moderation.content_reports(sla_deadline) WHERE status = 'pending';
CREATE INDEX idx_reports_created ON moderation.content_reports(created_at DESC);

COMMENT ON TABLE moderation.content_reports IS 'User reports - DSA Art. 14 + NetzDG § 3';
COMMENT ON COLUMN moderation.content_reports.is_offensichtlich_rechtswidrig IS 'NetzDG: Obviously illegal content (24h SLA requirement)';
COMMENT ON COLUMN moderation.content_reports.sla_deadline IS 'SLA deadline: 24h for obviously illegal, 7d for complex';
...
CREATE TABLE IF NOT EXISTS moderation.moderation_actions (
    ...
    -- Related Report
    related_report_id VARCHAR(36) REFERENCES moderation.content_reports(report_id),
...
CREATE INDEX idx_actions_content ON moderation.moderation_actions(content_type, content_id);
CREATE INDEX idx_actions_owner ON moderation.moderation_actions(content_owner_id);
CREATE INDEX idx_actions_type ON moderation.moderation_actions(action_type);
CREATE INDEX idx_actions_moderator ON moderation.moderation_actions(moderator_type, moderator_id);
CREATE INDEX idx_actions_report ON moderation.moderation_actions(related_report_id);
-- Index for appealable actions (removed CURRENT_TIMESTAMP as it's not IMMUTABLE)
CREATE INDEX idx_actions_appealable ON moderation.moderation_actions(is_appealable, appeal_deadline)
    WHERE is_appealable = TRUE;
CREATE INDEX idx_actions_created ON moderation.moderation_actions(created_at DESC);

COMMENT ON TABLE moderation.moderation_actions IS 'Moderation actions log - DSA Art. 15 Transparency requirement';
COMMENT ON COLUMN moderation.moderation_actions.reason IS 'REQUIRED - User-facing reason for action (DSA compliance)';
COMMENT ON COLUMN moderation.moderation_actions.is_appealable IS 'DSA: Users have right to appeal moderation decisions';

-- =====================================================
-- 3. USER VIOLATIONS (Trust & Safety)
-- =====================================================

CREATE TABLE IF NOT EXISTS moderation.user_violations (
...
CREATE INDEX idx_violations_user ON moderation.user_violations(user_id);
CREATE INDEX idx_violations_type ON moderation.user_violations(violation_type);
CREATE INDEX idx_violations_severity ON moderation.user_violations(severity);
CREATE INDEX idx_violations_created ON moderation.user_violations(created_at DESC);
-- Index for active violations (removed CURRENT_TIMESTAMP as it's not IMMUTABLE)
CREATE INDEX idx_violations_active ON moderation.user_violations(user_id, expires_at);

COMMENT ON TABLE moderation.user_violations IS 'User violations log - Trust & Safety 3-strike system';
COMMENT ON COLUMN moderation.user_violations.violation_points IS 'Points: 1-2 (minor), 3-5 (moderate), 6-8 (severe), 9-10 (critical)';
COMMENT ON COLUMN moderation.user_violations.expires_at IS 'Violations expire after 90d (minor), 180d (moderate), never (severe/critical)';

-- =====================================================
-- 4. AI MODERATION LOGS (Audit Trail)
-- =====================================================

CREATE TABLE IF NOT EXISTS moderation.ai_moderation_logs (
...
CREATE INDEX idx_ai_logs_content ON moderation.ai_moderation_logs(content_type, content_id);
CREATE INDEX idx_ai_logs_detection ON moderation.ai_moderation_logs(detection_type);
CREATE INDEX idx_ai_logs_flagged ON moderation.ai_moderation_logs(is_flagged);
CREATE INDEX idx_ai_logs_review_required ON moderation.ai_moderation_logs(human_review_required)
    WHERE human_review_required = TRUE;
CREATE INDEX idx_ai_logs_created ON moderation.ai_moderation_logs(created_at DESC);

COMMENT ON TABLE moderation.ai_moderation_logs IS 'AI moderation audit log - DSA Art. 24 (Algorithm Transparency)';
COMMENT ON COLUMN moderation.ai_moderation_logs.confidence_score IS 'AI confidence: 0-1 (threshold usually 0.7-0.8)';
COMMENT ON COLUMN moderation.ai_moderation_logs.processing_time_ms IS 'Performance metric for AI response time';
...
CREATE TABLE IF NOT EXISTS moderation.moderation_queue (
    ...
    related_report_id VARCHAR(36) REFERENCES moderation.content_reports(report_id),
    related_ai_log_id VARCHAR(36) REFERENCES moderation.ai_moderation_logs(log_id),
...
CREATE INDEX idx_queue_status ON moderation.moderation_queue(status);
CREATE INDEX idx_queue_priority ON moderation.moderation_queue(priority, created_at);
CREATE INDEX idx_queue_assigned ON moderation.moderation_queue(assigned_moderator_id);
CREATE INDEX idx_queue_sla ON moderation.moderation_queue(sla_deadline) WHERE status != 'completed';
CREATE INDEX idx_queue_content ON moderation.moderation_queue(content_type, content_id);

COMMENT ON TABLE moderation.moderation_queue IS 'Human moderation queue - Feature flag: human_moderation';
COMMENT ON COLUMN moderation.moderation_queue.sla_deadline IS 'SLA deadline for review (24h urgent, 7d normal)';
...
CREATE TRIGGER trg_reports_calculate_sla
    BEFORE INSERT ON moderation.content_reports
