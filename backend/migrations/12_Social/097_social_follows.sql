-- ============================================================================
-- Migration: 097_social_follows.sql
-- Description: Social Follow System - Follow/unfollow users
...
-- Date: 2026-01-16
-- ============================================================================
-- Previous number: 080 (12_Social) → Renumbered to 097 to resolve duplicates
-- Phase: Social Network Layer - Follow Relationships
--
-- Tables created:
...
CREATE TABLE IF NOT EXISTS social.social_follows (
...
CREATE INDEX idx_follows_follower ON social.social_follows(follower_user_id);
CREATE INDEX idx_follows_followed ON social.social_follows(followed_user_id);
CREATE INDEX idx_follows_approved ON social.social_follows(is_approved);
CREATE INDEX idx_follows_created ON social.social_follows(created_at DESC);

COMMENT ON TABLE social.social_follows IS 'Follow relationships between users - Feature flag: follow_system';
COMMENT ON COLUMN social.social_follows.is_approved IS 'For private accounts - follow request must be approved';
COMMENT ON CONSTRAINT chk_no_self_follow ON social.social_follows IS 'Users cannot follow themselves';

-- =====================================================
-- 2. USER FOLLOW STATS (Denormalized Counters)
-- =====================================================

CREATE TABLE IF NOT EXISTS social.social_user_stats (
...
CREATE INDEX idx_user_stats_followers ON social.social_user_stats(followers_count DESC);
CREATE INDEX idx_user_stats_engagement ON social.social_user_stats(engagement_rate DESC);

COMMENT ON TABLE social.social_user_stats IS 'Denormalized user statistics for performance (updated via triggers)';
COMMENT ON COLUMN social.social_user_stats.engagement_rate IS 'Engagement rate % for ranking/discovery';

-- =====================================================
-- 3. FOLLOW SUGGESTIONS (AI-Powered)
-- =====================================================

CREATE TABLE IF NOT EXISTS social.social_follow_suggestions (
...
CREATE INDEX idx_follow_suggestions_user ON social.social_follow_suggestions(user_id);
CREATE INDEX idx_follow_suggestions_suggested ON social.social_follow_suggestions(suggested_user_id);
CREATE INDEX idx_follow_suggestions_status ON social.social_follow_suggestions(status);
CREATE INDEX idx_follow_suggestions_expires ON social.social_follow_suggestions(expires_at);

COMMENT ON TABLE social.social_follow_suggestions IS 'AI-powered follow suggestions - Feature flag: follow_system';
COMMENT ON COLUMN social.social_follow_suggestions.confidence_score IS 'AI confidence score (0-1)';

-- =====================================================
-- 4. BLOCKED USERS (Privacy & Safety)
-- =====================================================

CREATE TABLE IF NOT EXISTS social.social_blocks (
...
CREATE INDEX idx_blocks_blocker ON social.social_blocks(blocker_user_id);
CREATE INDEX idx_blocks_blocked ON social.social_blocks(blocked_user_id);
CREATE INDEX idx_blocks_reason ON social.social_blocks(reason);

COMMENT ON TABLE social.social_blocks IS 'Blocked users (privacy & safety) - GDPR Art. 25 Privacy by Design';
COMMENT ON COLUMN social.social_blocks.reason IS 'Reason for blocking (optional, for Trust & Safety analytics)';

-- =====================================================
-- 5. MUTED USERS (Hide content without blocking)
-- =====================================================

CREATE TABLE IF NOT EXISTS social.social_mutes (
...
CREATE INDEX idx_mutes_muter ON social.social_mutes(muter_user_id);
CREATE INDEX idx_mutes_muted ON social.social_mutes(muted_user_id);
CREATE INDEX idx_mutes_until ON social.social_mutes(muted_until);

COMMENT ON TABLE social.social_mutes IS 'Muted users (hide content without blocking) - Less aggressive than blocking';
COMMENT ON COLUMN social.social_mutes.muted_until IS 'NULL = permanent mute, otherwise temporary';
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO social.social_user_stats (user_id)
...
CREATE TRIGGER trg_follows_init_stats
    BEFORE INSERT ON social.social_follows
...
BEGIN
    IF TG_OP = 'INSERT' AND NEW.is_approved = TRUE THEN
        ...
        UPDATE social.social_user_stats
...
CREATE TRIGGER trg_follows_update_counts
    AFTER INSERT OR DELETE OR UPDATE ON social.social_follows
...
CREATE TRIGGER trg_blocks_auto_unfollow
    AFTER INSERT ON social.social_blocks
