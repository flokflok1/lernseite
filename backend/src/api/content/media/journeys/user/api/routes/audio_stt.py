"""
Media Domain - Audio STT Routes (User Journey)

Speech-to-Text (STT) endpoints using OpenAI Whisper.

Endpoints:
- POST /audio/transcribe - Transcribe audio file
- POST /audio/transcribe-base64 - Transcribe base64 audio

Architecture: Journey-Based DDD (User)
Database: None (direct OpenAI API)
ISO 27001:2013 compliant - Audio transcription with secure temp file handling
"""
...
audio_stt_bp = Blueprint('audio_stt', __name__)
...
@audio_stt_bp.route('/audio/transcribe', methods=['POST'])
@token_required
...
@audio_stt_bp.route('/audio/transcribe-base64', methods=['POST'])
@token_required
def transcribe_audio_base64():
    ...
    Request Body:
        {
            ...
            "format": "webm",  // Audio format (required)
            "language": "de",  // Optional language code
            "prompt": "Context..."  // Optional context prompt
        }
    ...
    Response:
        ...
        400: Invalid request
             - NO_AUDIO_DATA: Audio data not provided
             - INVALID_FORMAT: Unsupported format
             - INVALID_BASE64: Invalid base64 encoding
             - FILE_TOO_LARGE: Data >25MB
        500: Transcription error
    ...
    Notes:
        - Useful for browser-recorded audio
        - Same format support as file upload
