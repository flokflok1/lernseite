"""
Auth Domain - Login Routes (Public Journey)
...
def login():
    ...
    try:
        ...
        # Authenticate user via AuthRepository
        user_dict = AuthRepository.find_user_by_email(login_data.email)
        ...
        # Verify password
        if not AuthRepository.verify_password(login_data.password, user_dict.get('password_hash', '')):
            # Record failed login attempt
            BruteForceProtection.record_failed_attempt(login_data.email, client_ip)
        ...
        if user_dict.get('two_factor_enabled', False):
            ...
            is_valid = AdminSetup.verify_2fa_code(
                user_dict.get('two_factor_secret'),
        ...
        AuditService.log_login_success(
            ...
            metadata={'2fa_used': user_dict.get('two_factor_enabled', False)}
        )

        # Create JWT tokens
        access_token = create_access_token(
            identity=str(user_dict['user_id']),
        ...
        if 'user_id' in user_dict and user_dict['user_id'] is not None:
...
def refresh():
    ...
    try:
        ...
        user_dict = AuthRepository.find_user_by_id(user_id)
        if not user_dict or user_dict.get('status') != 'active':
            return jsonify({
                'success': False,
