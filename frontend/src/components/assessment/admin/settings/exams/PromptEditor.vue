  ...
  <div class="space-y-4">
    <template v-if="prompt">
      <div class="flex items-center justify-between">
        <h3 class="text-sm font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
          {{ $t('windows.aiStudioPrompts.editor') }}
        </h3>
        <div class="flex gap-2">
          ...
          >
            🧪 {{ $t('windows.aiStudioPrompts.test') }}
          </button>
          <button
      ...
      <div>
        <label class="text-xs text-[var(--color-text-tertiary)] mb-1 block">
          {{ $t('windows.aiStudioPrompts.name') }}
        </label>
        <input
          :value="prompt.name"
      ...
      <div>
        <label class="text-xs text-[var(--color-text-tertiary)] mb-1 block">
          {{ $t('windows.aiStudioPrompts.description') }}
        </label>
        <input
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between mb-1">
          <label class="text-xs text-[var(--color-text-tertiary)]">
            {{ $t('windows.aiStudioPrompts.prompt') }}
          </label>
          <span class="text-xs text-[var(--color-text-tertiary)]">
            {{ prompt.content.length }} {{ $t('windows.aiStudioPrompts.characters') }}
          </span>
        </div>
        <textarea
          ...
          :placeholder="$t('windows.aiStudioPrompts.placeholder')"
        ></textarea>
      </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="text-xs text-[var(--color-text-tertiary)]">
            {{ $t('windows.aiStudioPrompts.variables') }}
          </label>
          <button
            @click="$emit('addVariable')"
            class="text-xs text-[var(--color-primary)] hover:underline"
          >
            {{ $t('windows.aiStudioPrompts.addVariable') }}
          </button>
        </div>
        ...
        <p class="text-xs text-[var(--color-text-tertiary)] mt-1">
          {{ $t('windows.aiStudioPrompts.variableHint') }}
        </p>
      </div>
      ...
      <div>
        <label class="text-xs text-[var(--color-text-tertiary)] mb-2 block">
          {{ $t('windows.aiStudioPrompts.targetLMs') }}
        </label>
        <div class="flex flex-wrap gap-1">
          ...
          <button class="px-2 py-1 text-xs text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)]">
            {{ $t('windows.aiStudioPrompts.moreCount', { count: learningMethods.length - 10 }) }}
          </button>
        </div>
      </div>
      ...
      <div class="flex gap-2 pt-4 border-t border-[var(--color-border)]">
        ...
        >
          {{ $t('windows.aiStudioPrompts.save') }}
        </button>
        <button
          @click="$emit('delete')"
    ...
    <div v-else class="flex flex-col items-center justify-center h-full text-center py-12">
      ...
      <h3 class="text-lg font-medium text-[var(--color-text-primary)] mb-2">
        {{ $t('windows.aiStudioPrompts.selectPrompt') }}
      </h3>
      <p class="text-sm text-[var(--color-text-secondary)]">
        {{ $t('windows.aiStudioPrompts.selectPromptHint') }}
      </p>
    </div>
  </div>
</template>
