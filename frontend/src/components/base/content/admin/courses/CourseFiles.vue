<!--
  Admin Course Files Window

  Manages course files (upload, list, delete) in a desktop window.
...
<template>
  <div class="admin-course-files-panel h-full flex flex-col bg-white dark:bg-gray-900">
    ...
    <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between animated-header-gradient">
      <div>
        <h2 class="text-sm font-bold text-gray-900 dark:text-white">{{ $t('features.courseFiles.title') }}</h2>
      ...
      <div class="flex items-center gap-2">
        ...
        >
          ...
          <span>{{ isUploading ? $t('features.courseFiles.uploading') : $t('features.courseFiles.upload') }}</span>
        </button>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-auto p-4">
      ...
      <div v-if="loading" class="flex items-center justify-center h-full">
        <div class="text-center">
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('features.courseFiles.loadingFiles') }}</p>
        </div>
      </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                :title="$t('features.courseFiles.download')"
              >
              ...
              <button
                ...
                :title="$t('features.courseFiles.delete')"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-else class="flex items-center justify-center h-full">
        <div class="text-center">
          ...
          <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-2">{{ $t('features.courseFiles.noFilesTitle') }}</h3>
          <p class="text-sm text-gray-600 dark:text-gray-400 mb-4 max-w-xs mx-auto">
            {{ $t('features.courseFiles.noFilesDesc') }}
          </p>
          <button
            @click="triggerFileUpload"
          ...
          >
            {{ $t('features.courseFiles.uploadFirst') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="files.length > 0" class="px-4 py-2 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
      <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
        <span>{{ $t('features.courseFiles.filesCount', { count: files.length }) }}</span>
        <span>{{ $t('features.courseFiles.totalSize', { size: formatFileSize(totalSize) }) }}</span>
      </div>
    </div>
  </div>
</template>
...
const courseTitle = computed(() => props.panel.payload?.courseTitle as string || t('common.course'))
...
const loadFiles = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || err.message || t('features.courseFiles.loadError')
  } finally {
    loading.value = false
  }
}
  ...
  try {
    ...
    for (const file of Array.from(selectedFiles)) {
      ...
      if (response.already_exists) {
        alert(response.message || t('features.courseFiles.fileExists', { name: file.name }))
      }
    }
    ...
    alert(t('features.courseFiles.uploadError') + ': ' + (err.response?.data?.error || err.message))
  } finally {
...
const deleteFile = async (file: CourseFile) => {
  ...
  const confirmed = confirm(t('features.courseFiles.deleteConfirm', { name: file.display_name || file.file_name }))
  if (!confirmed) return
  ...
  try {
    ...
    alert(t('features.courseFiles.deleteError') + ': ' + (err.response?.data?.error || err.message))
  }
}
...
const getCategoryLabel = (category: CourseFileCategory): string => {
  return t(`features.courseFiles.categories.${category}`) || category
}
