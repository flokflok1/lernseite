  ...
  <div class="modal-overlay" @click.self="$emit('cancel')">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h2 class="text-xl font-bold text-red-600">{{ $t('common.dialogs.deleteCategory') }}</h2>
        <button
          @click="$emit('cancel')"
      ...
      <div class="modal-body">
        <div class="flex items-start gap-4 mb-6">
          ...
          <div class="flex-1">
            <p class="text-[var(--color-text-primary)] mb-2">
              {{ $t('common.dialogs.deleteConfirm', { name: category.name }) }}
            </p>
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              <p class="text-sm font-medium text-yellow-800 mb-1">
                {{ $t('common.dialogs.cascadeWarning') }}
              </p>
              <p class="text-xs text-yellow-700">
                {{ $t('common.dialogs.subcategoriesWillBeDeleted', { count: totalDescendants }) }}
              </p>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              <p class="text-sm font-medium text-red-800 mb-1">
                {{ $t('common.dialogs.cannotDelete') }}
              </p>
              <p class="text-xs text-red-700 mb-2">
                {{ $t('common.dialogs.categoryHasCourses', { count: category.course_count || 0 }) }}
              </p>
              <p class="text-xs text-red-600 font-medium">
                {{ $t('common.dialogs.moveCoursesFirst') }}
              </p>
            </div>
          </div>
        </div>
        ...
        <label v-if="canDelete" class="flex items-start gap-2 cursor-pointer">
          ...
          <span class="text-sm text-[var(--color-text-primary)]">
            {{ $t('common.dialogs.understandCannotUndo') }}
            <span v-if="hasChildren" class="text-red-600 font-medium">
              {{ $t('common.dialogs.includingSubcategories', { count: totalDescendants }) }}
            </span>
          </span>
        </label>
        ...
        <div v-if="!canDelete" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p class="text-sm text-blue-700">
            <strong>{{ $t('common.tip') }}:</strong> {{ $t('common.dialogs.canDeactivateInstead') }}
          </p>
        </div>
      </div>
