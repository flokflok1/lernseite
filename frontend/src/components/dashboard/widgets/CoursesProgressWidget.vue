<template>
  <Card title="Lernfortschritt">
    <div class="space-y-4">
      <!-- Stats Grid -->
      <div class="grid grid-cols-2 gap-4">
        <div class="text-center p-3 bg-blue-50 rounded-lg">
          <p class="text-sm text-gray-600 mb-1">Kurse gestartet</p>
          <p class="text-3xl font-bold text-blue-600">{{ stats.started }}</p>
        </div>

        <div class="text-center p-3 bg-green-50 rounded-lg">
          <p class="text-sm text-gray-600 mb-1">Abgeschlossen</p>
      ...
      <div v-if="stats.totalLessons > 0" class="pt-3 border-t border-gray-200">
        <p class="text-sm text-gray-600 mb-1">Lektionen abgeschlossen</p>
...
const motivationalMessage = computed(() => {
  ...
  if (completed === started) {
    return '🎉 Fantastisch! Du hast alle Kurse abgeschlossen!'
  } else if (avgProgress >= 75) {
    return '💪 Super! Du bist auf einem sehr guten Weg!'
  } else if (avgProgress >= 50) {
    return '👍 Gut gemacht! Du bist über die Hälfte!'
  } else if (avgProgress >= 25) {
    return '🚀 Weiter so! Bleib am Ball!'
  } else {
    return '✨ Ein guter Start! Jeder Schritt zählt!'
  }
})
</script>
