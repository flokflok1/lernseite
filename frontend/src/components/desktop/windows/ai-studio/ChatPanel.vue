...
<template>
  ...
  >
    ...
    >
      ...
      <span class="text-xs text-[var(--color-text-tertiary)] writing-mode-vertical">KI-Chat</span>
    </button>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between p-3 border-b border-[var(--color-border)]">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3 class="font-medium text-[var(--color-text-primary)] text-sm">KI-Assistent</h3>
      ...
      <div v-if="currentContext" class="p-2 bg-[var(--color-primary-subtle)] text-xs">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[var(--color-text-secondary)]">Kontext: {{ currentContext }}</span>
        </div>
      </div>
      ...
      <div ref="messagesContainer" class="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div v-if="messages.length === 0" class="text-center py-6">
          ...
          <h4 class="font-medium text-[var(--color-text-primary)] mb-1">Hallo!</h4>
          <p class="text-sm text-[var(--color-text-secondary)]">
            Ich bin dein KI-Assistent. Stelle mir Fragen zum aktuellen Content oder zur Generierung.
          </p>
        </div>
      ...
      <div v-if="suggestions.length > 0" class="p-2 border-t border-[var(--color-border)]">
        <p class="text-xs text-[var(--color-text-tertiary)] mb-1">Vorschläge:</p>
...
const suggestions = ref<string[]>([
  'Was sind Teaching Steps?',
  'Wie generiere ich ein Video?',
...
function clearChat() {
  if (messages.value.length === 0 || confirm('Chat wirklich leeren?')) {
    messages.value = []
  }
}
