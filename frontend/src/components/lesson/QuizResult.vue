  ...
  <div class="quiz-result">
    ...
    <div class="text-center mb-8">
      ...
      <p class="text-gray-600">
        {{ result.is_exam ? 'Prüfung abgeschlossen' : 'Quiz abgeschlossen' }}
      </p>
    </div>
    ...
    >
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <div class="text-center">
          ...
          <div class="text-sm text-gray-600">Erreicht</div>
        </div>
      ...
      <div v-if="quiz?.passing_score_percentage" class="mt-6 pt-6 border-t border-gray-200 text-center">
        <p class="text-sm text-gray-600">
          <span class="font-medium">Zum Bestehen erforderlich:</span>
    ...
    <div v-if="quiz?.show_correct_answers && result.question_results.length > 0" class="mb-8">
      <h3 class="text-xl font-bold text-gray-900 mb-4">Detaillierte Ergebnisse</h3>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <div class="flex items-start gap-3 mb-3">
            ...
            >
              {{ questionResult.earned_points }}/{{ questionResult.max_points }} Punkte
            </div>
          </div>
          ...
          <div class="ml-11 space-y-2">
            <div class="text-sm">
              <span class="font-medium text-gray-700">Deine Antwort:</span>
            ...
            <div v-if="!questionResult.is_correct && questionResult.correct_answer" class="text-sm">
              <span class="font-medium text-gray-700">Richtige Antwort:</span>
            ...
            <div v-if="questionResult.explanation" class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-900">
              <span class="font-medium">💡 Erklärung:</span>
    ...
    <div v-else-if="result.is_exam" class="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
      <p class="text-sm text-yellow-800">
        <strong>Hinweis:</strong> Detaillierte Antworten sind bei Prüfungen nicht einsehbar.
      </p>
    </div>
    ...
    <div class="flex gap-4 justify-center">
      ...
      <Button
        ...
        @click="$emit('continue')"
      >
        {{ result.passed ? 'Weiter' : 'Zurück zur Übersicht' }}
      </Button>
    </div>
    ...
    <div v-if="result.is_exam" class="mt-8 pt-8 border-t border-gray-200">
      <p class="text-xs text-gray-500 text-center">
        Versuch abgeschlossen am {{ formatDate(result.submitted_at) }}
      </p>
    </div>
  </div>
</template>
...
const getQuestionText = (questionId: number): string => {
  ...
  return question?.question_text || 'Frage nicht gefunden'
}

const formatUserAnswer = (answer: QuizAnswerSubmission): string => {
  if (answer.answer_boolean !== undefined) {
    return answer.answer_boolean ? 'Richtig' : 'Falsch'
  }
...
const formatCorrectAnswer = (correctAnswer: any): string => {
  if (typeof correctAnswer === 'boolean') {
    return correctAnswer ? 'Richtig' : 'Falsch'
  }
