...
const checkPermission = async () => {
  try {
    ...
    errorMessage.value = 'Mikrofonzugriff verweigert. Bitte erlaube den Zugriff in den Browser-Einstellungen.'
...
const startRecording = async () => {
  ...
  try {
    ...
    errorMessage.value = err.message || 'Aufnahme konnte nicht gestartet werden'
    emit('error', errorMessage.value || 'Unbekannter Fehler')
  }
}
  ...
  <div class="audio-recorder bg-gray-50 dark:bg-gray-800 rounded-xl p-6">
    ...
    <p class="text-center text-sm text-gray-500 dark:text-gray-400 mt-4">
      <template v-if="!isRecording && !audioUrl">
        Klicke auf den roten Button um die Aufnahme zu starten
      </template>
      <template v-else-if="isRecording">
        {{ isPaused ? 'Aufnahme pausiert' : 'Aufnahme läuft...' }}
      </template>
      <template v-else>
        Aufnahme abgeschlossen ({{ formattedTime }})
      </template>
    </p>
  </div>
</template>
