...
<template>
  <div class="stats-tab p-6">
    ...
    <div v-if="isLoading" class="flex items-center justify-center py-20">
      ...
      <span class="ml-3 text-[var(--color-text-secondary)]">{{ $t('features.aiEditorStats.loading') }}</span>
    </div>

    <!-- Error State -->
    <div v-else-if="loadError" class="p-6 bg-red-50 dark:bg-red-900/20 rounded-xl text-center">
      <div class="text-4xl mb-3">❌</div>
      <h3 class="text-lg font-semibold text-red-600 dark:text-red-400 mb-2">{{ $t('features.aiEditorStats.errorTitle') }}</h3>
      ...
      <button @click="loadStats" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
        {{ $t('features.aiEditorStats.retry') }}
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="text-xl font-bold text-[var(--color-text-primary)]">{{ $t('features.aiEditorStats.title') }}</h2>
          <p class="text-sm text-[var(--color-text-secondary)] mt-1">
            {{ $t('features.aiEditorStats.subtitle') }}
          </p>
        </div>
        <div class="flex gap-2">
          ...
          >
            <option value="today">{{ $t('features.aiEditorStats.periods.today') }}</option>
            <option value="week">{{ $t('features.aiEditorStats.periods.week') }}</option>
            <option value="month">{{ $t('features.aiEditorStats.periods.month') }}</option>
            <option value="year">{{ $t('features.aiEditorStats.periods.year') }}</option>
          </select>
          <button
            @click="loadStats"
            :disabled="isLoading"
          ...
          >
            ...
            {{ $t('features.aiEditorStats.refresh') }}
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-4 mb-8">
        <div class="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl p-4 text-white">
          <div class="flex items-center justify-between mb-2">
            <span class="text-blue-100">{{ $t('features.aiEditorStats.totalTokens') }}</span>
      ...
      <div class="grid grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('features.aiEditorStats.usageByCategory') }}</h3>
          <div v-if="byCategory.length > 0" class="space-y-3">
            ...
            >
              ...
              <span class="text-xs text-[var(--color-text-tertiary)] w-16 text-right">
                {{ item.count }} {{ $t('features.aiEditorStats.times') }}
              </span>
            </div>
          </div>
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('features.aiEditorStats.noDataPeriod') }}
          </div>
        </div>

        <!-- Performance Metrics -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('features.aiEditorStats.performanceMetrics') }}</h3>
          <div class="grid grid-cols-2 gap-4">
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              ...
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('features.aiEditorStats.avgLatency') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-green-500">{{ performance.success_rate }}%</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('features.aiEditorStats.successRate') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-[var(--color-text-primary)]">{{ avgTokensPerRequest }}</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('features.aiEditorStats.tokensPerRequest') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-[var(--color-text-primary)]">{{ stats.total_sessions }}</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('features.aiEditorStats.sessions') }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-6">
        <!-- Model Usage -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('features.aiEditorStats.modelUsage') }}</h3>
          <div v-if="byModel.length > 0" class="space-y-3">
            ...
            >
              ...
              <div class="text-right">
                <div class="text-sm font-medium text-[var(--color-text-primary)]">{{ model.request_count }} {{ $t('features.aiEditorStats.requests') }}</div>
                <div class="text-xs text-[var(--color-text-tertiary)]">{{ formatNumber(model.tokens_used) }} Tokens</div>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('features.aiEditorStats.noModelUsage') }}
          </div>
        </div>

        <!-- Recent Activity -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('features.aiEditorStats.recentActivity') }}</h3>
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-[var(--color-text-tertiary)]">
                  {{ activity.model }} • {{ activity.tokens > 0 ? `${activity.tokens} ${$t('features.aiEditorStats.tokens')}` : '-' }}
                </div>
              </div>
          ...
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('features.aiEditorStats.noActivity') }}
          </div>
        </div>
      </div>
    </template>
...
const selectedPeriodLabel = computed(() => {
  const key = `features.aiEditorStats.periods.${selectedPeriod.value}`
  return t(key)
})
...
async function loadStats() {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.error?.message || t('features.aiEditorStats.loadError'))
    }
  } catch (error: any) {
    console.error('Failed to load stats:', error)
    loadError.value = error.response?.data?.error?.message || error.message || t('features.aiEditorStats.loadError')
  } finally {
