...
import { AudioRecorder } from '@/components/shared/audio'
import { analyzeOralExplanation, transcribeAudio } from '@/api/audio.api'
...
const examinerPersonaLabel = computed(() => {
  const personas: Record<string, string> = {
    formal: 'Formaler IHK-Prüfer',
    curious: 'Neugieriger Fragensteller',
    skeptical: 'Skeptischer Kritiker',
    supportive: 'Unterstützender Mentor',
    technical: 'Technischer Experte'
  }
  return personas[methodData.value.examiner_persona] || 'Prüfer'
})

// System prompt for the examiner
const examinerSystemPrompt = computed(() => {
  const personas: Record<string, string> = {
    formal: 'Du bist ein strenger, professioneller IHK-Prüfer. Du stellst präzise, prüfungsrelevante Fragen und erwartest strukturierte, vollständige Antworten.',
    curious: 'Du bist ein neugieriger, interessierter Prüfer. Du stellst viele Nachfragen, um das Verständnis des Prüflings zu vertiefen.',
    skeptical: 'Du bist ein kritischer Prüfer, der Aussagen hinterfragt. Du prüfst auf Lücken im Wissen und vertiefst bei Unklarheiten.',
    supportive: 'Du bist ein unterstützender Prüfer. Du gibst Hilfestellungen, wenn der Prüfling stockt, ohne die Antwort zu verraten.',
    technical: 'Du bist ein technischer Experte. Du fokussierst auf technische Details und korrekte Fachbegriffe.'
  }

  return `${personas[methodData.value.examiner_persona]}

Du führst eine mündliche Prüfung zum Thema "${methodData.value.topic}" durch.
Schwierigkeitsgrad: ${methodData.value.difficulty}
Format: ${methodData.value.format}

Regeln:
- Stelle immer nur EINE Frage auf einmal
- Reagiere auf die Antworten des Prüflings
- Stelle Nachfragen wenn nötig
- Bleibe im Kontext des Themas
...
const startExam = async () => {
  ...
  try {
    ...
    const response = await tutorChat({
      message: `Beginne die mündliche Prüfung zum Thema "${questionTopic}". Stelle die erste Frage.`,
      systemPrompt: examinerSystemPrompt.value,
      context: `Prüfungsformat: ${methodData.value.format}`
    })
    ...
    if (methodData.value.use_audio) {
      try {
        const audioUrl = await tutorTTS({ text: response.message, voice: 'onyx' })
        examinerMessages.value[examinerMessages.value.length - 1].audioUrl = audioUrl
        playAudio(audioUrl)
      } catch (e) {
    ...
    error.value = e.message || 'Fehler beim Starten der Prüfung'
  } finally {
...
const handleRecording = async (blob: Blob, duration: number) => {
  ...
  try {
    ...
    // Analyze the oral explanation
    const result = await analyzeOralExplanation(blob, {
      topic: currentQuestion.value,
      expectedPoints,
      language: 'de'
    })
    ...
    const examinerResponse = await tutorChat({
      message: `Der Prüfling hat geantwortet: "${result.transcription}"

Analyse der Antwort:
- Score: ${result.analysis.score}/100
- Abgedeckte Punkte: ${result.analysis.covered_points.join(', ') || 'Keine'}
- Fehlende Punkte: ${result.analysis.missing_points.join(', ') || 'Keine'}

Reagiere als Prüfer auf diese Antwort. Stelle eine Nachfrage oder gib eine kurze Bewertung.`,
      systemPrompt: examinerSystemPrompt.value,
      history: examinerMessages.value.slice(-6).map(m => ({
    ...
    // TTS for examiner response
    if (methodData.value.use_audio) {
      try {
        const audioUrl = await tutorTTS({ text: examinerResponse.message, voice: 'onyx' })
        examinerMessages.value[examinerMessages.value.length - 1].audioUrl = audioUrl
        playAudio(audioUrl)
      } catch (e) {
        console.warn('TTS failed:', e)
    ...
    error.value = e.message || 'Fehler bei der Analyse'
    phase.value = 'question'
  } finally {
    isLoading.value = false
  }
...
const handleTextAnswer = async () => {
  ...
  try {
    ...
    const examinerResponse = await tutorChat({
      message: `Der Prüfling hat geantwortet: "${userAnswer.value}"

Bewerte diese Antwort kurz und stelle dann eine Nachfrage oder gib an, dass der Themenblock abgeschlossen ist.`,
      systemPrompt: examinerSystemPrompt.value,
      history: examinerMessages.value.slice(-8).map(m => ({
        role: m.role === 'examiner' ? 'assistant' : 'user',
        content: m.content
      }))
    })
    ...
    error.value = e.message || 'Fehler bei der Verarbeitung'
    phase.value = 'question'
  } finally {
    isLoading.value = false
  }
...
const completeExam = async () => {
  ...
  isLoading.value = true
  try {
    const summaryResponse = await tutorChat({
      message: 'Die Prüfung ist beendet. Gib eine abschließende Gesamtbewertung der Prüfungsleistung.',
      systemPrompt: examinerSystemPrompt.value,
      history: examinerMessages.value.slice(-10).map(m => ({
        role: m.role === 'examiner' ? 'assistant' : 'user',
        content: m.content
      }))
    })
    ...
    if (methodData.value.use_audio) {
      try {
        const audioUrl = await tutorTTS({ text: summaryResponse.message, voice: 'onyx' })
        examinerMessages.value[examinerMessages.value.length - 1].audioUrl = audioUrl
        playAudio(audioUrl)
      } catch (e) {
        console.warn('TTS failed:', e)
  ...
  <div class="oral-explanation-lesson">
    ...
    <div v-if="phase === 'intro'" class="text-center py-8">
      <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 mb-6 text-left">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-3">Prüfungsdetails:</h3>
          <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-300">
            <li class="flex items-center gap-2">
              <span class="w-2 h-2 bg-purple-500 rounded-full"></span>
              Prüfer: {{ examinerPersonaLabel }}
            </li>
            <li class="flex items-center gap-2">
              <span class="w-2 h-2 bg-purple-500 rounded-full"></span>
              Themenblöcke: {{ totalBlocks }}
            </li>
            <li class="flex items-center gap-2">
              <span class="w-2 h-2 bg-purple-500 rounded-full"></span>
              Dauer: ca. {{ methodData.duration_minutes }} Minuten
            </li>
            <li v-if="methodData.use_audio" class="flex items-center gap-2">
              <span class="w-2 h-2 bg-green-500 rounded-full"></span>
              Sprachaufnahme aktiviert
            </li>
          </ul>
        </div>
    ...
    <div v-else-if="phase === 'feedback'" class="space-y-6">
      ...
      <div v-if="methodData.show_transcript && transcription" class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
        <h3 class="font-semibold text-gray-900 dark:text-white mb-2">Deine Antwort (Transkript):</h3>
      ...
      <div v-if="feedbackResult" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-semibold text-gray-900 dark:text-white">Bewertung</h3>
        ...
        <div v-if="feedbackResult.covered_points.length > 0" class="mb-4">
          <h4 class="text-sm font-medium text-green-700 dark:text-green-400 mb-2">Abgedeckte Punkte:</h4>
          <ul class="space-y-1">
        ...
        <div v-if="feedbackResult.missing_points.length > 0" class="mb-4">
          <h4 class="text-sm font-medium text-red-700 dark:text-red-400 mb-2">Fehlende Punkte:</h4>
          <ul class="space-y-1">
        ...
        <div v-if="feedbackResult.suggestions.length > 0">
          <h4 class="text-sm font-medium text-blue-700 dark:text-blue-400 mb-2">Verbesserungsvorschläge:</h4>
          <ul class="space-y-1">
      ...
      <div class="flex items-center justify-center gap-4">
        ...
        >
          {{ currentBlockIndex < totalBlocks - 1 ? 'Nächster Themenblock' : 'Prüfung abschließen' }}
        </button>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'complete'" class="text-center py-8">
      ...
      <div v-if="examinerMessages.length > 0" class="max-w-2xl mx-auto mb-6">
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 text-left">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-3">Abschließende Bewertung:</h3>
