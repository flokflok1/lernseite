<template>
  <div class="admin-course-list-content p-4">
    ...
    <div class="mb-3 flex justify-between items-center">
      <div>
        <h3 class="text-base font-bold text-[var(--color-text-primary)]">{{ $t('admin.courses.title') }}</h3>
        <p class="text-xs text-[var(--color-text-secondary)]">{{ $t('admin.courses.description') }}</p>
      </div>
    ...
    <div class="bg-[var(--color-surface)] rounded border border-[var(--color-border)]">
      ...
      <!-- Pagination -->
      <div v-if="adminStore.courses.length > 0" class="px-3 py-2 border-t border-[var(--color-border)] flex justify-between items-center">
        ...
        <div class="flex gap-1">
          ...
          <span class="px-2 py-1 text-xs">{{ currentPage }}/{{ Math.ceil(adminStore.coursesTotal / perPage) }}</span>
          <button
            @click="changePage(currentPage + 1)"
...
const publishCourse = async (courseId: string) => {
  if (!confirm(t('admin.courses.confirmPublish'))) return
  try {
...
const unpublishCourse = async (courseId: string) => {
  if (!confirm(t('admin.courses.confirmUnpublish'))) return
  try {
...
const archiveCourse = async (courseId: string) => {
  if (!confirm(t('admin.courses.confirmArchive'))) return
  try {
...
const unarchiveCourse = async (courseId: string) => {
  if (!confirm(t('admin.courses.confirmRestore'))) return
  try {
...
const permanentDeleteCourse = async (courseId: string, courseTitle: string) => {
  ...
  try {
    await adminStore.permanentDeleteCourse(courseId, 'Deleted by admin')
    loadCourses()
  } catch (error) {
...
const getStatusLabel = (status: string): string => {
  switch (status) {
    case 'draft': return t('admin.courses.draft')
    case 'published': return t('admin.courses.statusPublished')
    case 'archived': return t('admin.courses.archived')
...
onMounted(async () => {
  await Promise.allSettled([
    ...
    adminStore.loadCategoryTree().catch(err => console.warn('Failed to load categories:', err))
  ])
})
</script>
