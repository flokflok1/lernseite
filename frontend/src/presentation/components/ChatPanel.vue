...
<template>
  ...
  >
    ...
    >
      ...
      <span class="text-xs text-[var(--color-text-tertiary)] writing-mode-vertical">{{ $t('windows.aiEditorChat.title') }}</span>
    </button>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between p-3 border-b border-[var(--color-border)]">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3 class="font-medium text-[var(--color-text-primary)] text-sm">{{ $t('windows.aiEditorChat.assistant') }}</h3>
            <p class="text-xs text-[var(--color-text-tertiary)]">
              <span class="w-2 h-2 bg-green-500 rounded-full inline-block mr-1"></span>
              {{ $t('windows.aiEditorChat.online') }}
            </p>
          </div>
        </div>
        <div class="flex items-center gap-1">
          <button
            ...
            :title="$t('windows.aiEditorChat.clearChat')"
          >
            🗑️
          </button>
          <button
            @click="togglePanel"
            class="p-1.5 hover:bg-[var(--color-surface-secondary)] rounded transition-colors"
            :title="$t('windows.aiEditorChat.collapse')"
          >
            ›
          </button>
        </div>
      ...
      <div class="p-2 border-b border-[var(--color-border)]">
        <div v-if="actionsLoading" class="text-xs text-center text-[var(--color-text-tertiary)] py-1">
          {{ $t('windows.aiEditorChat.loadingActions') }}
        </div>
      ...
      <div v-if="currentContext" class="p-2 bg-[var(--color-primary-subtle)] text-xs">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[var(--color-text-secondary)]">{{ $t('windows.aiEditorChat.context') }}: {{ currentContext }}</span>
        </div>
      </div>
      ...
      <div ref="messagesContainer" class="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div v-if="messages.length === 0" class="text-center py-6">
          ...
          <h4 class="font-medium text-[var(--color-text-primary)] mb-1">{{ $t('windows.aiEditorChat.welcomeTitle') }}</h4>
          <p class="text-sm text-[var(--color-text-secondary)]">
            {{ $t('windows.aiEditorChat.welcomeText') }}
          </p>
        </div>
      ...
      <div v-if="suggestions.length > 0" class="p-2 border-t border-[var(--color-border)]">
        <p class="text-xs text-[var(--color-text-tertiary)] mb-1">{{ $t('windows.aiEditorChat.suggestions') }}</p>
      ...
      <div class="p-3 border-t border-[var(--color-border)]">
        <div class="flex gap-2">
          <textarea
            ...
            :placeholder="$t('windows.aiEditorChat.placeholder')"
            rows="2"
          ></textarea>
          <button
            @click="sendMessage"
        ...
        <div class="flex items-center justify-between mt-2 text-xs text-[var(--color-text-tertiary)]">
          <span>{{ $t('windows.aiEditorChat.enterToSend') }}</span>
...
import { getActionsByCategory, type AuthoringAction } from '@/infrastructure/api/clients/content'
...
const fallbackActions: QuickAction[] = [
  { action_id: 'fb-1', action_key: 'explain', label: t('windows.aiEditorChat.fallbackActions.explain.label'), icon: '📖', prompt_template: t('windows.aiEditorChat.fallbackActions.explain.prompt') },
  { action_id: 'fb-2', action_key: 'improve', label: t('windows.aiEditorChat.fallbackActions.improve.label'), icon: '✨', prompt_template: t('windows.aiEditorChat.fallbackActions.improve.prompt') },
  { action_id: 'fb-3', action_key: 'quiz_create', label: t('windows.aiEditorChat.fallbackActions.quiz.label'), icon: '❓', prompt_template: t('windows.aiEditorChat.fallbackActions.quiz.prompt') },
  { action_id: 'fb-4', action_key: 'summarize', label: t('windows.aiEditorChat.fallbackActions.summarize.label'), icon: '📝', prompt_template: t('windows.aiEditorChat.fallbackActions.summarize.prompt') }
]
...
const suggestions = ref<string[]>([
  t('windows.aiEditorChat.defaultSuggestions.teachingSteps'),
  t('windows.aiEditorChat.defaultSuggestions.videoGeneration'),
  t('windows.aiEditorChat.defaultSuggestions.modelComparison')
])
...
function clearChat() {
  if (messages.value.length === 0 || confirm(t('windows.aiEditorChat.confirmClear'))) {
    messages.value = []
  }
}
