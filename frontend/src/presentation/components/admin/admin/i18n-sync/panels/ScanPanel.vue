...
const statsData = computed(() => {
  ...
  return [
    {
      icon: '➕',
      label: t('admin.i18n.new_keys'),
      value: summary.new_keys,
      color: 'new'
    },
    {
      icon: '✏️',
      label: t('admin.i18n.changed_keys'),
      value: summary.changed_keys,
      color: 'changed'
    },
    {
      icon: '🗑️',
      label: t('admin.i18n.deleted_keys'),
      value: summary.deleted_keys,
      color: 'deleted'
    },
    {
      icon: '⚠️',
      label: t('admin.i18n.conflicted_keys'),
...
<template>
  <div class="scan-panel">
    ...
    <div class="panel-header">
      <h2>{{ $t('admin.i18n.scan_results') }}</h2>
    ...
    <div v-if="results?.summary" class="scan-status">
      <div class="status-row">
        <span class="status-label">{{ $t('admin.i18n.scan_status') }}:</span>
        <span
          class="status-badge"
      ...
      <div class="status-row">
        <span class="status-label">{{ $t('admin.i18n.scan_duration') }}:</span>
        <span class="status-value">{{ results.summary.scan_duration_ms }}ms</span>
      </div>

      <div class="status-row">
        <span class="status-label">{{ $t('admin.i18n.languages_affected') }}:</span>
    ...
    <div v-if="results?.summary" class="progress-section">
      <div class="progress-header">
        <span class="progress-label">{{ $t('admin.i18n.keys_processed') }}</span>
      ...
      <div class="progress-details">
        <span class="detail">
          {{ $t('admin.i18n.total_keys') }}: {{ results.summary.total_keys }}
        </span>
        <span class="detail">
          {{ $t('admin.i18n.total_changes') }}: {{ totalChanges }}
        </span>
      </div>
    </div>
    ...
    <div v-if="results?.next_action" class="next-steps">
      <h3>{{ $t('admin.i18n.next_steps') }}</h3>
      <div class="steps-content">
        <p v-if="results.next_action === 'REVIEW'">
          {{ $t('admin.i18n.next_review_desc') }}
        </p>
        <p v-else-if="results.next_action === 'APPLY_AUTO'">
          {{ $t('admin.i18n.next_apply_auto_desc') }}
        </p>
        <p v-else-if="results.next_action === 'APPLY_MANUAL'">
          {{ $t('admin.i18n.next_apply_manual_desc') }}
        </p>
      </div>
    </div>
    ...
    <div v-if="results?.summary?.error_message" class="error-section">
      <div class="error-box">
        ...
        <div class="error-content">
          <div class="error-title">{{ $t('admin.i18n.scan_error') }}</div>
