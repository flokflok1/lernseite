  ...
  <div class="admin-ai-kapitel-generator h-full flex flex-col bg-[var(--color-bg)]">
    ...
    <div class="p-4 border-b border-[var(--color-border)]">
      <h2 class="text-lg font-semibold text-[var(--color-text-primary)]">
        KI-Kapitel Generator
      </h2>
      <p class="text-sm text-[var(--color-text-secondary)] mt-1">
        Generiere Kapitel-Strukturen aus PDF-Dokumenten oder per Prompt
      </p>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-6">
      ...
      <div v-if="!selectedCourseId" class="space-y-4">
        ...
        <p class="text-sm text-[var(--color-text-secondary)]">
          Bitte öffne dieses Fenster über einen Kurs-Editor, um Kapitel zu generieren.
        </p>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="generationMode === 'prompt'" class="space-y-4">
          <div class="space-y-2">
            ...
            <textarea
              ...
              placeholder="Beschreibe die Themen und Struktur, die der Kurs haben soll..."
            ></textarea>
          </div>
        ...
        <div v-if="isGenerating" class="space-y-4">
          <div class="flex items-center gap-3">
            ...
            <span class="text-[var(--color-text-primary)]">Generiere Kapitel...</span>
          </div>
        ...
        <div v-if="generatedChapters.length > 0" class="space-y-4">
          <h3 class="text-sm font-medium text-[var(--color-text-primary)]">
            Generierte Kapitel ({{ generatedChapters.length }})
          </h3>
    ...
    <div class="p-4 border-t border-[var(--color-border)] flex justify-end gap-3">
      ...
      >
        {{ isApplying ? 'Wird angewendet...' : 'Kapitel übernehmen' }}
      </button>
      <button
        v-else
...
async function startGeneration() {
  ...
  try {
    ...
    for (let i = 0; i <= 100; i += 10) {
      ...
      progress.value = i
      statusMessage.value = i < 50 ? 'Analysiere Inhalt...' : 'Generiere Struktur...'
    }
    ...
    errorMessage.value = error.message || 'Fehler bei der Generierung'
  } finally {
...
async function applyChapters() {
  ...
  try {
    ...
    errorMessage.value = error.message || 'Fehler beim Anwenden der Kapitel'
  } finally {
