...
<template>
  <div class="stats-tab p-6">
    ...
    <div v-if="isLoading" class="flex items-center justify-center py-20">
      ...
      <span class="ml-3 text-[var(--color-text-secondary)]">{{ $t('windows.aiStudioStats.loading') }}</span>
    </div>

    <!-- Error State -->
    <div v-else-if="loadError" class="p-6 bg-red-50 dark:bg-red-900/20 rounded-xl text-center">
      <div class="text-4xl mb-3">❌</div>
      <h3 class="text-lg font-semibold text-red-600 dark:text-red-400 mb-2">{{ $t('windows.aiStudioStats.errorTitle') }}</h3>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="text-xl font-bold text-[var(--color-text-primary)]">{{ $t('windows.aiStudioStats.title') }}</h2>
          <p class="text-sm text-[var(--color-text-secondary)] mt-1">
            {{ $t('windows.aiStudioStats.subtitle') }}
          </p>
        </div>
        <div class="flex gap-2">
          ...
          >
            <option value="today">{{ $t('windows.aiStudioStats.periods.today') }}</option>
            <option value="week">{{ $t('windows.aiStudioStats.periods.week') }}</option>
            <option value="month">{{ $t('windows.aiStudioStats.periods.month') }}</option>
            <option value="year">{{ $t('windows.aiStudioStats.periods.year') }}</option>
          </select>
          <button
            @click="loadStats"
            :disabled="isLoading"
          ...
          >
            ...
            {{ $t('windows.aiStudioStats.refresh') }}
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-4 mb-8">
        <div class="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl p-4 text-white">
          <div class="flex items-center justify-between mb-2">
            <span class="text-blue-100">{{ $t('windows.aiStudioStats.totalTokens') }}</span>
      ...
      <div class="grid grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('windows.aiStudioStats.usageByCategory') }}</h3>
          <div v-if="byCategory.length > 0" class="space-y-3">
            ...
            >
              ...
              <span class="text-xs text-[var(--color-text-tertiary)] w-16 text-right">
                {{ item.count }} {{ $t('windows.aiStudioStats.times') }}
              </span>
            </div>
          </div>
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('windows.aiStudioStats.noDataPeriod') }}
          </div>
        </div>

        <!-- Performance Metrics -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('windows.aiStudioStats.performanceMetrics') }}</h3>
          <div class="grid grid-cols-2 gap-4">
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              ...
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('windows.aiStudioStats.avgLatency') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-green-500">{{ performance.success_rate }}%</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('windows.aiStudioStats.successRate') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-[var(--color-text-primary)]">{{ avgTokensPerRequest }}</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('windows.aiStudioStats.tokensPerRequest') }}</div>
            </div>
            <div class="text-center p-4 bg-[var(--color-surface-secondary)] rounded-lg">
              <div class="text-3xl font-bold text-[var(--color-text-primary)]">{{ stats.total_sessions }}</div>
              <div class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('windows.aiStudioStats.sessions') }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-6">
        <!-- Model Usage -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('windows.aiStudioStats.modelUsage') }}</h3>
          <div v-if="byModel.length > 0" class="space-y-3">
            ...
            >
              ...
              <div class="text-right">
                <div class="text-sm font-medium text-[var(--color-text-primary)]">{{ model.request_count }} {{ $t('windows.aiStudioStats.requests') }}</div>
                <div class="text-xs text-[var(--color-text-tertiary)]">{{ formatNumber(model.tokens_used) }} Tokens</div>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('windows.aiStudioStats.noModelUsage') }}
          </div>
        </div>

        <!-- Recent Activity -->
        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('windows.aiStudioStats.recentActivity') }}</h3>
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-[var(--color-text-tertiary)]">
                  {{ activity.model }} • {{ activity.tokens > 0 ? `${activity.tokens} ${$t('windows.aiStudioStats.tokens')}` : '-' }}
                </div>
              </div>
          ...
          <div v-else class="text-center py-8 text-[var(--color-text-tertiary)]">
            {{ $t('windows.aiStudioStats.noActivity') }}
          </div>
        </div>
      </div>
    </template>
...
async function loadStats() {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.error?.message || t('windows.aiStudioStats.loadError'))
    }
  } catch (error: any) {
    console.error('Failed to load stats:', error)
    loadError.value = error.response?.data?.error?.message || error.message || t('windows.aiStudioStats.loadError')
  } finally {
