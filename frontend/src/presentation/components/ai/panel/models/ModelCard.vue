  ...
  >
    <div class="flex items-start justify-between mb-3">
      ...
      <div class="flex flex-col gap-1 items-end">
        ...
        >
          {{ model.active ? $t('windows.aiEditorModels.active') : $t('windows.aiEditorModels.inactive') }}
        </span>
        <span v-if="model.is_default" class="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
          ⭐ {{ $t('windows.aiEditorModelCard.default') }}
        </span>
      </div>
    </div>
    ...
    <div class="mt-3 flex gap-2">
      ...
      >
        {{ model.active ? $t('windows.aiEditorModels.deactivate') : $t('windows.aiEditorModels.activate') }}
      </button>
      <button
      ...
      >
        {{ $t('windows.aiEditorModels.setDefault') }}
      </button>
    </div>
  </div>
</template>
