...
<template>
  <div class="flex items-center justify-between mb-4">
    <h3 class="text-sm font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
      {{ $t('windows.aiEditorModels.availableModels') }} ({{ modelCount }})
    </h3>
    <div class="flex gap-2">
      ...
      >
        <option value="">{{ $t('windows.aiEditorModels.allProviders') }}</option>
      ...
      <select
        ...
        class="px-3 py-1.5 text-sm bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg"
      >
        <option value="">{{ $t('windows.aiEditorModels.allCategories') }}</option>
      ...
      <input
        ...
        type="text"
        :placeholder="$t('windows.aiEditorModels.search')"
