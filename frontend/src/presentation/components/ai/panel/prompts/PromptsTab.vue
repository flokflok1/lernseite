...
<template>
  <div class="prompts-tab p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-[var(--color-text-primary)]">{{ $t('aiEditorPrompts.title') }}</h2>
        <p class="text-sm text-[var(--color-text-secondary)] mt-1">
          {{ $t('aiEditorPrompts.subtitle') }}
        </p>
      </div>
      <div class="flex gap-2">
        ...
        >
          <span>📥</span> {{ $t('aiEditorPrompts.import') }}
        </button>
        <button
          @click="exportPrompts"
          class="px-4 py-2 bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-primary)] rounded-lg hover:bg-[var(--color-surface-secondary)] transition-colors flex items-center gap-2"
        >
          <span>📤</span> {{ $t('aiEditorPrompts.export') }}
        </button>
        <button
        ...
        >
          <span>+</span> {{ $t('aiEditorPrompts.newPrompt') }}
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      <div class="col-span-1 space-y-4">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorPrompts.promptsCount', { count: category.count }) }}</div>
                </div>
              </div>
        ...
        <div class="mt-6">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] uppercase tracking-wide mb-3">
            {{ $t('aiEditorPrompts.learningMethods') }}
          </h3>
      ...
      <div class="col-span-1 space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
            {{ $t('aiEditorPrompts.prompts') }}
          </h3>
          <input
            v-model="promptSearch"
            type="text"
            :placeholder="$t('aiEditorPrompts.search')"
        ...
        <div class="space-y-2 max-h-[500px] overflow-y-auto">
          ...
          >
            <div class="flex items-start justify-between mb-2">
              ...
              >
                {{ prompt.isActive ? $t('aiEditorPrompts.active') : $t('aiEditorPrompts.inactive') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-2 text-xs text-[var(--color-text-tertiary)]">
              <span>{{ prompt.variables.length }} {{ $t('aiEditorPrompts.variables') }}</span>
              <span>•</span>
              <span>{{ prompt.tokens }} {{ $t('aiEditorPrompts.tokens') }}</span>
              <span>•</span>
          ...
          <div v-if="filteredPrompts.length === 0" class="text-center py-8">
            ...
            <p class="text-[var(--color-text-secondary)]">{{ $t('aiEditorPrompts.noPromptsFound') }}</p>
          </div>
        </div>
      </div>
...
import { TestPromptModal, PromptEditor } from '@/presentation/components/studio/assessment/panel/settings/exams'
...
const promptCategories = computed(() => [
  { id: 'content', name: t('aiEditorPrompts.categories.content'), emoji: '📝', count: 8 },
  { id: 'video', name: t('aiEditorPrompts.categories.video'), emoji: '🎬', count: 4 },
  { id: 'quiz', name: t('aiEditorPrompts.categories.quiz'), emoji: '❓', count: 6 },
  { id: 'feedback', name: t('aiEditorPrompts.categories.feedback'), emoji: '💬', count: 3 },
  { id: 'translation', name: t('aiEditorPrompts.categories.translation'), emoji: '🌍', count: 2 },
  { id: 'summary', name: t('aiEditorPrompts.categories.summary'), emoji: '📋', count: 3 }
])
...
function createNewPrompt() {
  selectedPrompt.value = {
    ...
    name: t('aiEditorPrompts.newPrompt'),
    description: '',
    content: '',
...
function deletePrompt() {
  if (!selectedPrompt.value || !confirm(t('aiEditorPrompts.confirmDelete'))) return
...
function duplicatePrompt() {
  ...
  const newPrompt = {
    ...selectedPrompt.value,
    id: Date.now().toString(),
    name: `${selectedPrompt.value.name} ${t('aiEditorPrompts.copy')}`
  }
...
function addVariable() {
  const name = prompt(t('aiEditorPrompts.variableNamePrompt'))
