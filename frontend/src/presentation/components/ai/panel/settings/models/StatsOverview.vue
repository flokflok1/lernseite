...
<template>
  <div v-if="stats" class="grid grid-cols-5 gap-4 mb-6">
    <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center">
      ...
      <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorModels.totalModels') }}</div>
    </div>
    <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center">
      <div class="text-2xl font-bold text-green-600">{{ stats.active_models || 0 }}</div>
      <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorModels.activeModels') }}</div>
    </div>
    <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center">
      <div class="text-2xl font-bold text-blue-600">{{ stats.providers || 0 }}</div>
      <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorModels.providers') }}</div>
    </div>
    <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center">
      <div class="text-2xl font-bold text-purple-600">{{ stats.categories || 0 }}</div>
      <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorModels.categories') }}</div>
    </div>
    <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center">
      <div class="text-2xl font-bold text-orange-600">{{ stats.default_models || 0 }}</div>
      <div class="text-xs text-[var(--color-text-tertiary)]">{{ $t('aiEditorModels.defaultModels') }}</div>
    </div>
  </div>
</template>
