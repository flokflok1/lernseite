<!--
  SyncResultBanner - Sync Operation Result Display
...
<template>
  ...
  >
    ...
    <div v-if="result.details" class="mt-2 text-sm text-[var(--color-text-secondary)]">
      {{ $t('windows.aiStudioModels.added') }}: {{ result.details.added || 0 }} |
      {{ $t('windows.aiStudioModels.updated') }}: {{ result.details.updated || 0 }} |
      {{ $t('windows.aiStudioModels.total') }}: {{ result.details.synced || 0 }}
    </div>
  </div>
</template>
