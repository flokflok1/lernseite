          ...
          <div class="space-y-6 max-w-2xl">
            ...
            <div>
              ...
              <input
                ...
                :placeholder="$t('features.kapitelEditor.titlePlaceholder')"
              />
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                {{ $t('features.kapitelEditor.descriptionLabel') }}
              </label>
              <textarea
                ...
                rows="6"
                class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
                :placeholder="$t('features.kapitelEditor.descriptionPlaceholder')"
              ></textarea>
            </div>
        ...
        <div v-else-if="activeTab === 'videos'" class="p-6">
          <div class="max-w-4xl">
            ...
            <div v-if="videos.length === 0" class="text-center py-12 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg">
              ...
              <p class="text-[var(--color-text-secondary)]">{{ $t('features.kapitelEditor.noVideos') }}</p>
              <p class="text-sm text-[var(--color-text-secondary)] mt-1">{{ $t('features.kapitelEditor.noVideosHint') }}</p>
            </div>
                ...
                <div class="flex items-start gap-4">
                  <div class="flex-1">
                    <input
                      v-model="video.title"
                      type="text"
                      :placeholder="$t('features.kapitelEditor.videoTitlePlaceholder')"
                    ...
                    <input
                      v-model="video.url"
                      type="text"
                      :placeholder="$t('features.kapitelEditor.videoUrlPlaceholder')"
              ...
              <div>
                ...
                <p class="text-sm text-[var(--color-text-secondary)] mt-1">
                  <span v-if="learningMethods.length > 0">
                    {{ learningMethods.filter(m => m.published).length }} {{ $t('features.kapitelEditor.published') }},
                    {{ learningMethods.filter(m => !m.published).length }} {{ $t('features.kapitelEditor.draft') }}
                  </span>
                  <span v-else>{{ $t('features.kapitelEditor.noMethods') }}</span>
                </p>
              </div>
              <button
