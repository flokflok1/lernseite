<template>
  <Modal
    :show="show"
    :title="$t('admin.courseEditor.selectCourse')"
    size="lg"
    @close="$emit('close')"
  >
    <template #default>
      ...
      <div class="mb-6">
        <div class="relative">
          <input
            v-model="searchQuery"
            type="text"
            :placeholder="$t('admin.courseEditor.searchPlaceholder')"
      ...
      <div class="mb-6">
        ...
        >
          ➕ {{ $t('admin.courseEditor.createNewCourse') }}
        </button>
      </div>
      ...
      <div class="relative mb-6">
        ...
        <div class="relative flex justify-center text-sm">
          <span class="px-2 bg-white text-gray-500">{{
            $t('admin.courseEditor.existingCourses', 'Existing Courses')
          }}</span>
        </div>
      </div>
      ...
      <div v-else-if="filteredCourses.length === 0" class="text-center py-8">
        <p class="text-gray-500 mb-2">{{
          $t('admin.courseEditor.noCourses')
        }}</p>
        <p class="text-sm text-gray-400">{{
          $t('admin.courseEditor.noCoursesDescription')
        }}</p>
      </div>
      ...
      <div v-else class="space-y-3 max-h-96 overflow-y-auto">
        ...
        >
          ...
          <div class="flex gap-2">
            ...
            >
              <span v-if="!isOpening">🤖 {{ $t('admin.courseEditor.openWithAI') }}</span>
            ...
            >
              <span v-if="!isOpening">✏️ {{ $t('admin.courseEditor.openWithManual') }}</span>
              <span v-else>⏳ {{ $t('common.loading') }}</span>
            </button>
          </div>
        </div>
...
import * as courseApi from '@/infrastructure/api/clients/content/courses.api'
...
async function loadCourses() {
  ...
  try {
    ...
    error.value = t('admin.courseEditor.loadError', 'Failed to load courses. Please try again.')
...
function handleCreateNew() {
  ...
  const title = window.prompt(t('admin.courseEditor.enterCourseName', 'Enter course name:'))
