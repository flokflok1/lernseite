      ...
      <div class="space-y-5 max-w-2xl">
        ...
        <div class="p-4 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
          ...
          <p class="mt-2 text-xs text-[var(--color-text-secondary)]">
            {{ form.ai_model_override ? $t('courseCreate.form.courseUsesModel', { model: form.ai_model_override }) : $t('courseCreate.form.usesDefaultModel') }}
          </p>
        </div>
...
const createCourse = async (): Promise<void> => {
  ...
  try {
    ...
    alert(t('courseCreate.errors.createError') + ': ' + (error.message || t('common.unknownError')))
  } finally {
