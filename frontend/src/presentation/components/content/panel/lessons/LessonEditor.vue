      ...
      <div class="space-y-6 max-w-3xl mx-auto">
        ...
        <div>
          ...
          <input
            ...
            :placeholder="$t('lessonEditor.titlePlaceholder')"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
            :placeholder="$t('lessonEditor.descriptionPlaceholder')"
          ></textarea>
        </div>
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
          >
            <option value="">{{ $t('lessonEditor.selectType') }}</option>
            <option value="text">📄 {{ $t('lessonEditor.types.text') }}</option>
            <option value="video">🎥 {{ $t('lessonEditor.types.video') }}</option>
            <option value="quiz">❓ {{ $t('lessonEditor.types.quiz') }}</option>
            <option value="interactive">🎮 {{ $t('lessonEditor.types.interactive') }}</option>
            <option value="exercise">💪 {{ $t('lessonEditor.types.exercise') }}</option>
            <option value="ai">🤖 {{ $t('lessonEditor.types.ai') }}</option>
            <option value="exam">📝 {{ $t('lessonEditor.types.exam') }}</option>
          </select>
        </div>
        ...
        <div v-if="form.lesson_type" class="border-t border-[var(--color-border)] pt-6">
          <h3 class="text-lg font-semibold text-[var(--color-text-primary)] mb-4">
            {{ lessonTypeLabel }} - {{ $t('lessonEditor.content') }}
          </h3>
          ...
          <div v-if="form.lesson_type === 'text'" class="space-y-4">
            <div>
              ...
              <textarea
                ...
                :placeholder="$t('lessonEditor.textPlaceholder')"
              ></textarea>
              <p class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('lessonEditor.markdownHint') }}</p>
            </div>
          </div>
          ...
          <div v-else-if="form.lesson_type === 'video'" class="space-y-4">
            <div>
              ...
              <input
                ...
                required
                class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
                :placeholder="$t('lessonEditor.videoUrlPlaceholder')"
              />
              <p class="text-xs text-[var(--color-text-tertiary)] mt-1">{{ $t('lessonEditor.videoUrlHint') }}</p>
            </div>

            <div>
              <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                {{ $t('lessonEditor.videoDescription') }}
              </label>
              <textarea
                ...
                rows="4"
                class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
                :placeholder="$t('lessonEditor.videoDescPlaceholder')"
              ></textarea>
            </div>
          </div>
          ...
          <div v-else-if="form.lesson_type === 'quiz'" class="space-y-4">
            ...
            <div v-for="(question, qIdx) in form.content.questions" :key="qIdx" class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-4 space-y-3">
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1">
                  <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                    {{ $t('lessonEditor.question') }} {{ qIdx + 1 }}
                  </label>
                  <input
                ...
                <button
                  ...
                  :title="$t('lessonEditor.deleteQuestion')"
                >
          ...
          <div v-else-if="form.lesson_type === 'exam'" class="space-y-4">
            <div class="flex items-center justify-between mb-3">
              <h4 class="text-sm font-medium text-[var(--color-text-primary)]">
                {{ $t('lessonEditor.examQuestions') }} ({{ form.content.exam_questions.length }})
              </h4>
              <button
                @click="addExamQuestion"
                class="px-3 py-1.5 bg-[var(--color-primary)] text-white text-sm rounded-lg hover:bg-[var(--color-primary-hover)]"
              >
                {{ $t('lessonEditor.addExamQuestion') }}
              </button>
            </div>
            ...
            <div v-if="form.content.exam_questions.length === 0" class="text-center py-8 text-[var(--color-text-tertiary)]">
              {{ $t('lessonEditor.noExamQuestions') }}
            </div>
          </div>
          ...
          <div v-else class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p class="text-sm text-blue-700">
              ℹ️ {{ $t('lessonEditor.notImplemented', { type: lessonTypeLabel }) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="validationErrors.length > 0" class="bg-red-50 border border-red-200 rounded-lg p-4">
          <p class="text-sm font-semibold text-red-800 mb-2">{{ $t('lessonEditor.validation.title') }}</p>
...
const lessonTypeLabel = computed(() => {
  ...
  if (!typeKey) return ''
  return t(`lessonEditor.typeLabels.${typeKey}`) || typeKey
})
...
const validationErrors = computed(() => {
  ...
  if (!form.value.title.trim()) {
    errors.push(t('lessonEditor.validation.titleRequired'))
  }

  if (!form.value.lesson_type) {
    errors.push(t('lessonEditor.validation.typeRequired'))
  }

  if (form.value.lesson_type === 'video' && !form.value.content.video_url) {
    errors.push(t('lessonEditor.validation.videoUrlRequired'))
  }

  if (form.value.lesson_type === 'quiz') {
    if (form.value.content.questions.length === 0) {
      errors.push(t('lessonEditor.validation.minOneQuestion'))
    }

    form.value.content.questions.forEach((q, idx) => {
      if (!q.text.trim()) {
        errors.push(t('lessonEditor.validation.questionTextMissing', { num: idx + 1 }))
      }
      if (q.options.length < 2) {
        errors.push(t('lessonEditor.validation.minTwoAnswers', { num: idx + 1 }))
      }
      if (q.correct_indices.length === 0) {
        errors.push(t('lessonEditor.validation.minOneCorrect', { num: idx + 1 }))
      }
    })
  }
...
const saveLesson = async () => {
  if (validationErrors.value.length > 0) {
    ...
    errorMessage.value = t('lessonEditor.validation.failed')
    setTimeout(() => {
  ...
  try {
    if (isNewLesson.value && !lesson.value) {
      // Create new lesson
      if (!moduleId.value) {
        throw new Error(t('lessonEditor.errors.moduleIdMissing'))
      }
