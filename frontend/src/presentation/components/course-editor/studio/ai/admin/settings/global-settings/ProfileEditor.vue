...
<template>
  <div class="profile-editor">
    <div v-if="!profile && !isCreating" class="editor-empty">
      <p>{{ $t('features.aiEditorGlobalSettings.selectProfileHint') }}</p>
    </div>

    <template v-else>
      <div class="editor-header">
        <h4>{{ isCreating ? $t('features.aiEditorGlobalSettings.newProfileTitle') : $t('features.aiEditorGlobalSettings.editProfileTitle') }}</h4>
      </div>

      <div class="editor-form">
        <div class="form-row">
          <label>{{ $t('features.aiEditorGlobalSettings.key') }}</label>
          <input v-if="isCreating" :value="formData.key" @input="updateField('key', $event)" type="text" :placeholder="$t('features.aiEditorGlobalSettings.keyPlaceholder')" class="form-input" />
          <span v-else class="form-value mono">{{ formData.key }}</span>
        </div>
        <div class="form-row">
          <label>{{ $t('features.aiEditorGlobalSettings.name') }}</label>
          <input :value="formData.name" @input="updateField('name', $event)" type="text" :placeholder="$t('features.aiEditorGlobalSettings.namePlaceholder')" class="form-input" />
        </div>
        <div class="form-row">
          <label>{{ $t('features.aiEditorGlobalSettings.description') }}</label>
          <input :value="formData.description" @input="updateField('description', $event)" type="text" :placeholder="$t('features.aiEditorGlobalSettings.descriptionPlaceholder')" class="form-input" />
        </div>

        <div class="categories-section">
          <h5>{{ $t('features.aiEditorGlobalSettings.modelsPerCategory') }}</h5>
          <div class="category-grid">
            <div v-for="category in categories" :key="category" class="category-item">
              ...
              <select :value="formData.models[category]" @change="updateModel(category, $event)" class="category-select">
                <option value="">{{ $t('features.aiEditorGlobalSettings.notSet') }}</option>
        ...
        <div class="editor-actions">
          <button v-if="!isCreating && !profile?.is_default" @click="$emit('delete')" class="btn-danger">{{ $t('features.aiEditorGlobalSettings.delete') }}</button>
          <div class="action-spacer"></div>
          <button v-if="!isCreating && !profile?.is_default" @click="$emit('setDefault')" class="btn-secondary">{{ $t('features.aiEditorGlobalSettings.setAsDefault') }}</button>
          <button @click="$emit('cancel')" class="btn-secondary">{{ $t('features.aiEditorGlobalSettings.cancel') }}</button>
          <button @click="$emit('save')" :disabled="saving" class="btn-primary">{{ saving ? $t('features.aiEditorGlobalSettings.saving') : $t('features.aiEditorGlobalSettings.save') }}</button>
        </div>
      </div>
    </template>
  </div>
