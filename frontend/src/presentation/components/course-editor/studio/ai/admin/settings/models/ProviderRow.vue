  ...
  <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-[var(--color-text-tertiary)]">
            {{ modelCount }} {{ $t('features.aiEditorModels.models') }}
            <span v-if="provider.last_validated" class="ml-2">
              | {{ $t('features.aiEditorModels.validated') }}: {{ formatDate(provider.last_validated) }}
            </span>
          </p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-[var(--color-text-secondary)]">
          {{ provider.has_api_key ? $t('features.aiEditorModels.configured') : $t('features.aiEditorModels.notConfigured') }}
        </span>
        <span
        ...
        >
          {{ $t('features.aiEditorModels.active') }}
        </span>
      </div>
    </div>
    <div class="mt-3 flex gap-2">
      <input
        ...
        :placeholder="provider.has_api_key ? '••••••••••••••••' : $t('features.aiEditorModels.apiKeyPlaceholder')"
      />
      <button
      ...
      >
        💾 {{ $t('features.aiEditorModels.save') }}
      </button>
      <button
        @click="$emit('test')"
        :disabled="isTesting"
