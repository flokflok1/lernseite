...
<template>
  <div class="lernmethoden-tab p-6">
    ...
    <div v-if="!course" class="empty-state">
      <div class="icon">🧩</div>
      <h3>{{ $t('features.lernmethodenTab.noCourseSelected') }}</h3>
      <p>{{ $t('features.lernmethodenTab.selectCourseHint') }}</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h2 class="text-xl font-bold text-[var(--color-text-primary)]">
            🧩 {{ $t('features.lernmethodenTab.title', { title: course.title }) }}
          </h2>
          <p class="text-sm text-[var(--color-text-secondary)] mt-1">
            {{ $t('features.lernmethodenTab.stats', { total: methodStats.total, lessons: methodStats.lessons }) }}
          </p>
        </div>
      ...
      <div v-if="loading" class="loading-state">
        ...
        <p>{{ $t('features.lernmethodenTab.loading') }}</p>
      </div>
        ...
        <div v-for="group in methodGroups" :key="group.code" class="method-group">
          ...
          <div class="group-header" @click="toggleGroup(group.code)">
            <div class="group-info">
              ...
              <span class="group-code">{{ $t('features.lernmethodenTab.groupLabel', { code: group.code }) }}</span>
            </div>
            <div class="group-stats">
              <span class="group-count">{{ $t('features.lernmethodenTab.usedCount', { count: getGroupMethodCount(group.code) }) }}</span>
              <svg
                class="expand-icon"
      ...
      <div v-if="selectedChapter || selectedLesson" class="context-info">
        <span class="context-label">{{ $t('features.lernmethodenTab.activeContext') }}</span>
...
import { MethodCard } from '@/components/studio/assessment/admin/settings/exams'
...
function getGroupDefinitions() {
  return [
    { code: 'A', name: t('features.lernmethodenTab.groups.explaining'), icon: '📖' },
    { code: 'B', name: t('features.lernmethodenTab.groups.practice'), icon: '🎯' },
    { code: 'C', name: t('features.lernmethodenTab.groups.exam'), icon: '📝' },
    { code: 'D', name: t('features.lernmethodenTab.groups.pro'), icon: '⭐' },
    { code: 'E', name: t('features.lernmethodenTab.groups.it'), icon: '💻' },
    { code: 'F', name: t('features.lernmethodenTab.groups.collaborative'), icon: '👥' }
  ]
}
