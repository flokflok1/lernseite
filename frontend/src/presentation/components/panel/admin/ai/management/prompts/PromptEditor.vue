  ...
  <div class="space-y-4">
    <template v-if="prompt">
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between mb-1">
          <label class="text-xs text-[var(--color-text-tertiary)]">
            {{ $t('aiEditorPrompts.prompt') }}
          </label>
          <span class="text-xs text-[var(--color-text-tertiary)]">
            {{ prompt.content.length }} {{ $t('aiEditorPrompts.characters') }}
          </span>
        </div>
        <textarea
          ...
          :placeholder="$t('aiEditorPrompts.placeholder')"
        ></textarea>
      </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="text-xs text-[var(--color-text-tertiary)]">
            {{ $t('aiEditorPrompts.variables') }}
          </label>
          <button
            @click="$emit('addVariable')"
            class="text-xs text-[var(--color-primary)] hover:underline"
          >
            {{ $t('aiEditorPrompts.addVariable') }}
          </button>
        </div>
        ...
        <p class="text-xs text-[var(--color-text-tertiary)] mt-1">
          {{ $t('aiEditorPrompts.variableHint') }}
        </p>
      </div>
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-1">
          ...
          <button class="px-2 py-1 text-xs text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)]">
            {{ $t('aiEditorPrompts.moreCount', { count: learningMethods.length - 10 }) }}
          </button>
        </div>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center h-full text-center py-12">
      ...
      <h3 class="text-lg font-medium text-[var(--color-text-primary)] mb-2">
        {{ $t('aiEditorPrompts.selectPrompt') }}
      </h3>
      <p class="text-sm text-[var(--color-text-secondary)]">
        {{ $t('aiEditorPrompts.selectPromptHint') }}
      </p>
    </div>
  </div>
</template>
