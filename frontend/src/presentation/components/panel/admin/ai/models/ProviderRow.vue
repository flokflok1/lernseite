  ...
  <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-[var(--color-text-tertiary)]">
            {{ modelCount }} {{ $t('aiEditorModels.models') }}
            <span v-if="provider.last_validated" class="ml-2">
              | {{ $t('aiEditorModels.validated') }}: {{ formatDate(provider.last_validated) }}
            </span>
          </p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-[var(--color-text-secondary)]">
          {{ provider.has_api_key ? $t('aiEditorModels.configured') : $t('aiEditorModels.notConfigured') }}
        </span>
        <span
    ...
    <div class="mt-3 flex gap-2">
      <input
        ...
        :placeholder="provider.has_api_key ? '••••••••••••••••' : $t('aiEditorModels.apiKeyPlaceholder')"
      />
      <button
