...
<template>
  <div class="community-review">
    <h2>{{ $t('panel.community.reviewQueue') }}</h2>

    <!-- Review Queue Stats -->
    <div class="review-stats">
      <div class="stat-badge">
        <span class="stat-badge__label">{{ $t('panel.community.pending') }}</span>
    ...
    <div class="review-container">
      ...
      <div class="review-list">
        <div class="review-list__header">
          <h3>{{ $t('panel.community.submittedCourses') }}</h3>
      ...
      <div class="review-detail">
        <div v-if="selectedReview" class="review-panel">
          ...
          <div class="review-actions">
            ...
            >
              {{ $t('panel.community.reject') }}
            </button>
          </div>

          <!-- Review Notes -->
          <div class="review-notes">
            <label>{{ $t('panel.community.reviewNotes') }}:</label>
            <textarea
              v-model="reviewNotes"
              rows="6"
              :placeholder="$t('panel.community.notesPlaceholder')"
              class="textarea"
            ></textarea>
            <p class="notes-hint" v-if="!reviewNotes.trim()">
              {{ $t('panel.community.rejectRequiresNotes') }}
            </p>
          </div>
        </div>

        <div v-else class="empty-state">
          <p>{{ $t('panel.community.selectCourse') }}</p>
        </div>
      </div>
    </div>
  </div>
