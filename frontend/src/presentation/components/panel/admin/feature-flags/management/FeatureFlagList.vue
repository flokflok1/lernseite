  ...
  <div class="feature-flag-list">
    ...
    <div class="list-controls">
      ...
      <select v-model="filterEnabled" class="filter-select">
        <option value="">{{ $t('panel.featureFlags.allFlags') }}</option>
        <option value="enabled">{{ $t('panel.featureFlags.enabledOnly') }}</option>
        <option value="disabled">{{ $t('panel.featureFlags.disabledOnly') }}</option>
      </select>
      <select v-model="filterCategory" class="filter-select">
        <option value="">{{ $t('panel.featureFlags.allCategories') }}</option>
    ...
    <div class="table-container">
      <table class="flags-table">
        <thead>
          <tr>
            <th>{{ $t('panel.featureFlags.featureCode') }}</th>
            <th>{{ $t('panel.featureFlags.description') }}</th>
            <th>{{ $t('panel.featureFlags.category') }}</th>
            <th>{{ $t('panel.featureFlags.status') }}</th>
            <th>{{ $t('panel.featureFlags.rolloutPercentage') }}</th>
          ...
          <tr v-for="flag in filteredFlags" :key="flag.feature_id" class="flag-row">
            ...
            <td class="actions">
              ...
              <button
                ...
                :title="$t('panel.featureFlags.startRollout')"
              >
                {{ $t('panel.featureFlags.rollout') }}
              </button>
            </td>
          </tr>
        </tbody>
      ...
      <div v-if="filteredFlags.length === 0" class="empty-state">
        <p v-if="flags.length === 0">
          {{ $t('panel.featureFlags.noFlags') }}
        </p>
        <p v-else>
          {{ $t('panel.featureFlags.noMatchingFlags') }}
        </p>
      </div>
    </div>
    ...
    <div class="summary">
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.totalFlags') }}:</span>
        <span class="summary-value">{{ flags.length }}</span>
      </div>
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.enabledCount') }}:</span>
        <span class="summary-value">{{ enabledCount }}</span>
      </div>
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.disabledCount') }}:</span>
...
import type { FeatureFlag } from '@/application/services/api/panel-admin'
