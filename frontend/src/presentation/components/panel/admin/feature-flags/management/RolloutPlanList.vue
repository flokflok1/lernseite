  ...
  <div class="rollout-plan-list">
    ...
    <div class="list-controls">
      <select v-model="filterStatus" class="filter-select">
        <option value="">{{ $t('panel.featureFlags.allStatuses') }}</option>
        <option value="in_progress">{{ $t('panel.featureFlags.inProgress') }}</option>
        <option value="completed">{{ $t('panel.featureFlags.completed') }}</option>
        <option value="paused">{{ $t('panel.featureFlags.paused') }}</option>
        <option value="rolled_back">{{ $t('panel.featureFlags.rolledBack') }}</option>
      </select>
    </div>
    ...
    <div class="table-container">
      <table class="plans-table">
        <thead>
          <tr>
            <th>{{ $t('panel.featureFlags.featureName') }}</th>
            <th>{{ $t('panel.featureFlags.status') }}</th>
            <th>{{ $t('panel.featureFlags.currentPercentage') }}</th>
            <th>{{ $t('panel.featureFlags.targetPercentage') }}</th>
            <th>{{ $t('panel.featureFlags.startDate') }}</th>
            <th>{{ $t('panel.featureFlags.estimatedEndDate') }}</th>
          ...
          <tr v-for="plan in filteredPlans" :key="plan.plan_id" class="plan-row">
            ...
            <td class="actions">
              <button
                ...
                :title="$t('panel.featureFlags.updatePercentage')"
              >
                {{ $t('panel.featureFlags.update') }}
              </button>
              <button
                v-if="plan.status === 'in_progress'"
                class="btn-small btn-warning"
                @click="$emit('pause', plan.plan_id)"
                :title="$t('panel.featureFlags.pauseRollout')"
              >
                {{ $t('panel.featureFlags.pause') }}
              </button>
              <button
                v-if="plan.status === 'paused'"
                class="btn-small btn-success"
                @click="$emit('resume', plan.plan_id)"
                :title="$t('panel.featureFlags.resumeRollout')"
              >
                {{ $t('panel.featureFlags.resume') }}
              </button>
              <button
                v-if="plan.status !== 'rolled_back'"
                class="btn-small btn-danger"
                @click="$emit('rollback', plan.plan_id)"
                :title="$t('panel.featureFlags.rollbackFeature')"
              >
                {{ $t('panel.featureFlags.rollback') }}
              </button>
            </td>
          </tr>
        </tbody>
      ...
      <div v-if="filteredPlans.length === 0" class="empty-state">
        <p v-if="plans.length === 0">
          {{ $t('panel.featureFlags.noRolloutPlans') }}
        </p>
        <p v-else>
          {{ $t('panel.featureFlags.noMatchingPlans') }}
        </p>
      </div>
    </div>
    ...
    <div v-if="selectedPlan" class="dialog-overlay" @click="selectedPlan = null">
      <div class="dialog" @click.stop>
        <h2>{{ $t('panel.featureFlags.updatePercentage') }}</h2>
        <p>{{ $t('panel.featureFlags.updatePercentageHelp') }}</p>
    ...
    <div class="summary">
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.activeRollouts') }}:</span>
        <span class="summary-value">{{ activeRolloutsCount }}</span>
      </div>
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.completedRollouts') }}:</span>
        <span class="summary-value">{{ completedRolloutsCount }}</span>
      </div>
      <div class="summary-item">
        <span class="summary-label">{{ $t('panel.featureFlags.pausedRollouts') }}:</span>
...
import type { RolloutPlan } from '@/application/services/api/panel-admin'
