...
const categories = computed(() => [
  { code: 'audio', name: t('panel.lmRouting.categories.audio'), emoji: '🎵', color: 'bg-green-500' },
  { code: 'chat', name: t('panel.lmRouting.categories.chat'), emoji: '💬', color: 'bg-blue-500' },
  { code: 'embedding', name: t('panel.lmRouting.categories.embedding'), emoji: '📊', color: 'bg-cyan-500' },
  { code: 'image', name: t('panel.lmRouting.categories.image'), emoji: '🖼️', color: 'bg-pink-500' },
  { code: 'moderation', name: t('panel.lmRouting.categories.moderation'), emoji: '🛡️', color: 'bg-gray-500' },
  { code: 'realtime', name: t('panel.lmRouting.categories.realtime'), emoji: '⚡', color: 'bg-yellow-500' },
  { code: 'reasoning', name: t('panel.lmRouting.categories.reasoning'), emoji: '🧠', color: 'bg-orange-500' },
  { code: 'video', name: t('panel.lmRouting.categories.video'), emoji: '🎬', color: 'bg-red-500' },
  { code: 'vision', name: t('panel.lmRouting.categories.vision'), emoji: '👁️', color: 'bg-indigo-500' }
])
...
const loadData = async () => {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : t('panel.lmRouting.loadError')
  } finally {
    loading.value = false
  }
}
...
const applyPreset = async (preset: 'cheap' | 'medium' | 'expensive') => {
  ...
  try {
    ...
    successMessage.value = t('panel.lmRouting.slotsConfigured', { count: result.configured })
    await loadData()
...
const saveDefaultModel = async (categoryCode: string, modelId: number | null) => {
  ...
  try {
    ...
    successMessage.value = t('panel.lmRouting.defaultModelSet', { category: categoryCode })
    setTimeout(() => { successMessage.value = null }, 2000)
  } catch (err) {
    error.value = err instanceof Error ? err.message : t('panel.lmRouting.saveError')
  } finally {
...
const getGroupName = (key: string) => t(`panel.lmRouting.groupNames.${key}`)
...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('panel.lmRouting.title') }}</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        {{ $t('panel.lmRouting.subtitle') }}
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div class="mb-8 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
        <div class="flex items-center justify-between">
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('panel.lmRouting.quickSetup') }}</span>
          <div class="flex gap-2">
            ...
            >
              {{ presetRunning === 'cheap' ? '...' : '💰 ' + $t('panel.lmRouting.presetCheap') }}
            </button>
            <button
            ...
            >
              {{ presetRunning === 'medium' ? '...' : '⚖️ ' + $t('panel.lmRouting.presetMedium') }}
            </button>
            <button
            ...
            >
              {{ presetRunning === 'expensive' ? '...' : '👑 ' + $t('panel.lmRouting.presetPremium') }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="mb-8">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ $t('panel.lmRouting.defaultModelsPerCategory') }}</h2>
      ...
      <div>
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ $t('panel.lmRouting.lmRequirements') }}</h2>

        <div class="space-y-6">
          <div v-for="(lms, groupKey) in lmGroups" :key="groupKey">
            <template v-if="lms.length > 0">
              <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                {{ $t('panel.lmRouting.group', { key: groupKey, name: getGroupName(groupKey as string) }) }}
              </h3>
      ...
      <div class="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <h4 class="font-medium text-blue-900 dark:text-blue-300 mb-2">{{ $t('panel.lmRouting.legend') }}</h4>
        <div class="flex flex-wrap gap-4 text-sm text-blue-700 dark:text-blue-400">
          <span>🎵 {{ $t('panel.lmRouting.legendAudio') }}</span>
          <span>💬 {{ $t('panel.lmRouting.legendChat') }}</span>
          <span>🎬 {{ $t('panel.lmRouting.legendVideo') }}</span>
          <span>⚡ {{ $t('panel.lmRouting.legendRealtime') }}</span>
          <span>🧠 {{ $t('panel.lmRouting.legendReasoning') }}</span>
        </div>
        <div class="mt-2 text-xs text-blue-600 dark:text-blue-500">
          {{ $t('panel.lmRouting.legendColoredRequired') }} | {{ $t('panel.lmRouting.legendGrayOptional') }}
        </div>
      </div>
    </template>
  </div>
