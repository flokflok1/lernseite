    ...
    <div class="preview-header">
      ...
      <div class="header-actions">
        <button
          ...
          @click="downloadFile"
          class="action-btn"
          :title="$t('panel.actions.download')"
        >
          ⬇️
        </button>
        <button
    ...
    <div class="preview-content">
      ...
      <template v-else-if="previewType === 'file'">
        ...
        <div v-else class="state-unsupported">
          ...
          <button @click="downloadFile" class="download-btn">Herunterladen</button>
        </div>
      </template>
        ...
        <div class="chapter-preview">
          <div class="chapter-header-card">
            ...
            <div class="chapter-stats">
              <span>📄 {{ chapterLessons.length }} Lektionen</span>
            </div>
          </div>
...
const headerTitle = computed(() => {
  if (previewType.value === 'file') return file.value?.name || 'Datei'
  if (previewType.value === 'chapter') return chapter.value?.title || 'Kapitel'
  if (previewType.value === 'lesson') return lesson.value?.title || 'Lektion'
  return 'Vorschau'
})
...
const headerMeta = computed(() => {
  ...
  if (previewType.value === 'chapter') {
    return `${chapterLessons.value.length} Lektionen`
  }
  if (previewType.value === 'lesson') {
    return lesson.value?.duration_minutes ? `${lesson.value.duration_minutes} Min` : 'Lektion'
  }
  return ''
})
...
async function loadContent() {
  ...
  try {
    if (previewType.value === 'file') {
      ...
      error.value = 'Unbekannter Vorschau-Typ'
      loading.value = false
    }
  } catch (err: any) {
...
async function loadFile() {
  ...
  try {
    ...
    error.value = err.response?.data?.error || 'Datei konnte nicht geladen werden'
  } finally {
    loading.value = false
  }
}
