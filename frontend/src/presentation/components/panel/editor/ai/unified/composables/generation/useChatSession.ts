...
export function useChatSession() {
  ...
  async function sendMessage(
    ...
    fileIds?: string[],
    contextHint?: string,
    focusContext?: { type: 'chapter' | 'lesson'; id: string },
    qualityLevel?: string
    ...
    try {
      const raw = await apiSendChat(session.value.sessionId, messageForAI, {
        ...
        focus_chapter_id: focusContext?.type === 'chapter' ? focusContext.id : undefined,
        focus_lesson_id: focusContext?.type === 'lesson' ? focusContext.id : undefined,
      ...
      // Show content validation warnings
      if (response.content_validation && !response.content_validation.valid) {
        const errors = response.content_validation.errors || []
        const warnings = response.content_validation.warnings || []
        if (errors.length > 0 || warnings.length > 0) {
          const parts: string[] = []
          if (errors.length > 0) parts.push(`${errors.length} Fehler`)
          if (warnings.length > 0) parts.push(`${warnings.length} Warnungen`)
          messages.value.push({
            id: `msg-${Date.now()}-val`,
            role: 'system',
            content: `Validierung: ${parts.join(', ')}`,
            timestamp: new Date().toISOString(),
          })
        }
      }
