...
export function usePlanMode(qualityLevelRef?: { selectedLevel: { value: string } }) {
  const { t } = useI18n()

  /** Resolve the current quality level (defaults to 'standard'). */
  const _ql = () => qualityLevelRef?.selectedLevel?.value || 'standard'
  ...
  // ── Chat enhancement state (#3, #6, #7) ─────────────────────
  const lastDiffSummary = ref<string[]>([])
  ...
  async function generatePhase1(courseId: string, topic?: string, fileIds?: string[], language?: string) {
    isCreating.value = true
    error.value = null
    try {
  ...
  async function confirmPhase() {
    ...
    try {
      if (currentPhase.value === 1) {
        const plan = await advanceToPhase2(planId, _ql())
        currentPlan.value = _normalizePlan(plan)
        chapters.value = plan.chapters || []
        currentPhase.value = 2
      } else if (currentPhase.value === 2) {
        const plan = await advanceToPhase3(planId, _ql())
        currentPlan.value = _normalizePlan(plan)
  ...
  async function sendPlanChatMessage(message: string): Promise<string> {
    ...
    error.value = null
    lastDiffSummary.value = []
    validationWarnings.value = []
    try {
      chatMessages.value.push({ role: 'user', content: message })
      const result = await sendPlanChat(currentPlan.value.plan_id, message, _ql())

      // Build enriched assistant message with diff + warnings
      let displayMessage = result.assistant_message

      // #6: Append diff summary if plan was changed
      if (result.diff_summary?.length) {
        lastDiffSummary.value = result.diff_summary
        displayMessage += '\n\n📋 Änderungen: ' + result.diff_summary.join(' | ')
      }

      // #3: Append validation warnings
      if (result.validation_warnings?.length) {
        validationWarnings.value = result.validation_warnings
        displayMessage += '\n\n⚠️ ' + result.validation_warnings.join(' | ')
      }
      ...
      // #7: Track undo availability
      canUndo.value = result.has_undo || false
  ...
  async function undoLastPatch(): Promise<boolean> {
    if (!currentPlan.value || !canUndo.value) return false

    error.value = null
    try {
      const result = await undoPlanChat(currentPlan.value.plan_id)
      if (result.plan) {
        currentPlan.value = _normalizePlan(result.plan)
        courseMeta.value = result.plan.course_meta || courseMeta.value
        chapters.value = result.plan.chapters || chapters.value
      ...
      chatMessages.value.push({
        role: 'assistant',
        content: '↩️ Letzte Änderung rückgängig gemacht.',
      })
      return true
    } catch (e: unknown) {
      _handleError(e, 'undoLastPatch')
      return false
    }
  }
