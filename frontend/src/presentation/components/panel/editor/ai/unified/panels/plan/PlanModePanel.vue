...
import StepDetailPanel from '../skill/StepDetailPanel.vue'
...
/** Convert snake_case skill_code to localized name via i18n. */
function skillLabel(code: string): string {
  // generate_theory_sheet → generateTheorySheet
  const camel = code.replace(/_([a-z])/g, (_, c) => c.toUpperCase())
  const key = `aiEditor.skills.${camel}.name`
  const translated = t(key)
  // If key not found, i18n returns the key itself — fall back to readable code
        ...
        <div v-for="(phase, phaseIdx) in plan.phases" :key="phase.phase_id" class="phase">
          ...
          <div class="steps-list">
            ...
            >
              ...
              <div class="step-body">
                <div class="step-title">{{ step.target_title || skillLabel(step.skill_code) }}</div>
                <div v-if="step.skill_code" class="step-skill">
                  {{ skillLabel(step.skill_code) }}
                </div>
              </div>
