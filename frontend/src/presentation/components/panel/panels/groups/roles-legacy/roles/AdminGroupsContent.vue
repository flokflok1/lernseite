<template>
  <div class="admin-roles-content">
    ...
    <div class="mb-6 flex gap-3">
      ...
      >
        <span>📋</span>
        <span>{{ $t('panel.groups.templates.title') }}</span>
      </button>
      ...
      >
        <span>➕</span>
        <span>{{ $t('panel.groups.createCustom') }}</span>
      </button>
    </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full my-8">
        <div class="p-6">
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">
            {{ currentRole ? $t('panel.groups.edit') : $t('panel.groups.create') }}
          </h2>
    ...
    >
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <div v-if="roleToDelete && roleToDelete.user_count && roleToDelete.user_count > 0" class="mb-4">
          ...
          >
            <option value="">{{ $t('panel.groups.selectTargetRole') }}</option>
            <option
              v-for="role in rolesStore.roles.filter((r) => r.role_id !== roleToDelete?.role_id)"
              :key="role.role_id"
              :value="role.role_id"
            >
              {{ role.display_name }} ({{ role.user_count || 0 }} {{ $t('panel.groups.userCount') }})
            </option>
          </select>
        </div>
