    ...
    <div v-if="showCreateUserModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $t('panel.users.createUser') }}</h3>
        </div>
        <div class="px-6 py-4">
          <!-- Error Message -->
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ $t('panel.users.role') }} *</label>
            <select
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
          ...
          >
            {{ isCreatingUser ? '...' : $t('panel.users.createUser') }}
          </button>
        </div>
      </div>
    </div>
...
const confirmCreateUser = async () => {
  ...
  try {
    ...
    createUserError.value = null
    await panelStore.createUser(createUserForm.value)
