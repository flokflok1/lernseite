  ...
  <div class="rpg-skill-tree">
    ...
    <div class="skill-tree-container">
      ...
      <svg class="skill-connections" viewBox="0 0 300 280">
        <line x1="75" y1="80" x2="75" y2="160"
          :class="['skill-line', { active: isSkillUnlocked('focus') }]" />
        <line x1="75" y1="80" x2="225" y2="160"
          :class="['skill-line', { active: isSkillUnlocked('focus') }]" />
        <line x1="75" y1="200" x2="150" y2="260"
          :class="['skill-line', { active: isSkillUnlocked('logic') }]" />
        <line x1="225" y1="200" x2="150" y2="260"
      ...
      <div class="skill-nodes">
        ...
        <div class="skill-row row-1">
          <div class="skill-node" :class="getNodeClass('focus')" @click="handleSkillClick('focus')">
            <div class="node-glow"></div>
            <div class="node-icon">🎯</div>
            <div class="node-name">Hyper Focus</div>
            <div class="node-cost" v-if="!isSkillUnlocked('focus')">{{ getSkillCost('focus') }} ⭐</div>
          </div>
          <div class="skill-node" :class="getNodeClass('golddigger')" @click="handleSkillClick('golddigger')">
            <div class="node-glow"></div>
            <div class="node-icon">💰</div>
            <div class="node-name">Goldgraeber</div>
            <div class="node-cost" v-if="!isSkillUnlocked('golddigger')">{{ getSkillCost('golddigger') }} ⭐</div>
          </div>
        </div>

        <!-- Row 2: Logic & Endurance -->
        <div class="skill-row row-2">
          <div class="skill-node" :class="getNodeClass('logic')" @click="handleSkillClick('logic')">
            <div class="node-glow"></div>
            <div class="node-icon">🧠</div>
            <div class="node-name">Logik-Meister</div>
            <div class="node-cost" v-if="!isSkillUnlocked('logic')">{{ getSkillCost('logic') }} ⭐</div>
          </div>
          <div class="skill-node" :class="getNodeClass('endurance')" @click="handleSkillClick('endurance')">
            <div class="node-glow"></div>
            <div class="node-icon">💪</div>
            <div class="node-name">Ausdauer</div>
            <div class="node-cost" v-if="!isSkillUnlocked('endurance')">{{ getSkillCost('endurance') }} ⭐</div>
          </div>
        </div>

        <!-- Row 3: Codemaster -->
        <div class="skill-row row-3">
          <div class="skill-node ultimate" :class="getNodeClass('codemaster')" @click="handleSkillClick('codemaster')">
            <div class="node-glow"></div>
            <div class="node-icon">⚔️</div>
            <div class="node-name">Code-Meister</div>
            <div class="node-cost" v-if="!isSkillUnlocked('codemaster')">{{ getSkillCost('codemaster') }} ⭐</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <Transition name="fade">
      <SkillInfoPanel
        v-if="selectedSkill"
        :skill="selectedSkill"
        :unlockable="canUnlock(selectedSkill.id)"
        :locked-reason="getLockedReason(selectedSkill.id)"
        :requirement-names="selectedSkill.requires ? getRequirementNames(selectedSkill.requires) : ''"
        @unlock="handleUnlock"
      />
    </Transition>
...
import SkillInfoPanel from './SkillInfoPanel.vue'
...
.skill-header {
  ...
  align-items: center;
  padding: 16px 20px;
  background: linear-gradient(145deg, var(--color-background) 0%, var(--color-surface) 100%);
...
.points-value { font-size: 16px; font-weight: 700; color: #8b5cf6; }
...
.skill-nodes { position: relative; z-index: 1; }
...
.skill-row.row-3 { margin-bottom: 0; }
...
.skill-node.ultimate { width: 90px; height: 90px; }
.skill-node.locked { opacity: 0.5; filter: grayscale(0.8); }
...
.skill-node.selected { transform: scale(1.05); box-shadow: 0 0 24px var(--color-primary); }
...
.legend-dot.locked { background: var(--color-background); border-color: var(--color-border); opacity: 0.5; }
.legend-dot.available { background: var(--color-background); border-color: var(--color-primary); }
.legend-dot.unlocked { background: linear-gradient(135deg, rgba(251, 191, 36, 0.3) 0%, var(--color-background) 100%); border-color: #fbbf24; }
.legend-label { font-size: 11px; color: var(--color-text-muted); }
...
@media (max-width: 640px) {
  ...
  .skill-node.ultimate { width: 80px; height: 80px; }
  .node-icon { font-size: 24px; }
  .node-name { font-size: 9px; }
  .skill-legend { flex-wrap: wrap; gap: 12px; }
}
</style>
