<template>
  <div class="detailed-steps">
    <div class="steps-header">
      <h3 class="steps-title">{{ $t('lesson.steps.title') }}</h3>
      <div v-if="steps.length > 0" class="step-counter">
        {{ $t('lesson.steps.stepOf', { current: currentStep + 1, total: steps.length }) }}
      </div>
    </div>

    <div v-if="steps.length === 0" class="no-steps">
      <p>{{ $t('lesson.steps.noSteps') }}</p>
      <Button v-if="canGenerate" variant="primary" @click="$emit('generate')">
        {{ $t('lesson.steps.generateSteps') }}
      </Button>
    </div>
