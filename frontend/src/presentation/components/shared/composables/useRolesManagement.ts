...
export function useRolesManagement() {
  ...
  async function loadRoles() {
    loading.value = true
    try {
      roles.value = await groupsApi.getRoles(true)
    } catch (e) {
  ...
  async function loadPermissions() {
    try {
      permissions.value = await groupsApi.getPermissionsGrouped()
    } catch (e) {
  ...
  async function selectRole(role: Role) {
    ...
    try {
      const [fullRole, users] = await Promise.all([
        groupsApi.getRole(role.role_id),
        groupsApi.getRoleUsers(role.role_id, 50)
      ])
  ...
  async function createRole(data: {
    ...
    try {
      await groupsApi.createRole(data)
      await loadRoles()
      return true
    } catch (e) {
  ...
  async function updateRole(roleId: number, data: Partial<Role>) {
    try {
      await groupsApi.updateRole(roleId, data)
      await loadRoles()
  ...
  async function deleteRole(role: Role) {
    ...
    try {
      await groupsApi.deleteRole(role.role_id)
  ...
  async function setRolePermissions(roleId: number, permissionIds: number[]) {
    try {
      await groupsApi.setRolePermissions(roleId, permissionIds)
      if (selectedRole.value?.role_id === roleId) {
        const fullRole = await groupsApi.getRole(roleId)
        selectedRole.value = fullRole
      }
      return true
    } catch (e) {
