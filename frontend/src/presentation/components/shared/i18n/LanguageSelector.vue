<template>
  <div class="language-selector" :class="{ 'is-open': isOpen }">
    ...
    >
      <span :class="['fi', `fi-${getCountryCode(currentLanguage)}`]"></span>
...
import type { LanguageProgress } from '@/infrastructure/api/clients/public'
import 'flag-icons/css/flag-icons.min.css'

// Map language codes to country codes (ISO 3166-1-alpha-2)
const languageToCountryMap: Record<string, string> = {
  en: 'gb',  // English -> Great Britain
  // Most language codes match country codes (de, pl, fr, es, etc.)
}
...
// Computed
// Show all active languages with priority <= 3 as primary (de, pl, en)
const primaryLanguages = computed(() =>
  languages.value.filter(l => l.active && l.priority <= 3).sort((a, b) => a.priority - b.priority)
)

// Other languages are those with lower priority (community translations)
...
/* Flag icon styling - uses flag-icons package */
.fi {
  width: 1.25rem;
  height: 1rem;
