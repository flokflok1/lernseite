  ...
  <div class="lm-plugin-management">
    ...
    <div class="header">
      ...
      <button
        ...
        class="btn-scan"
      >
        {{ isLoading ? $t('admin.plugins.scanning') : $t('admin.plugins.scanPlugins') }}
      </button>
    </div>
    ...
    <div class="tabs">
      <button
        ...
        class="tab-btn"
      >
        {{ $t('admin.plugins.pending') }} ({{ pendingCount }})
      </button>
      <button
        ...
        class="tab-btn"
      >
        {{ $t('admin.plugins.active') }} ({{ activeCount }})
      </button>
    </div>
    ...
    <div v-else-if="displayedPlugins.length === 0" class="empty-state">
      <p>{{ activeTab === 'pending' ? $t('admin.plugins.noPendingPlugins') : $t('admin.plugins.noActivePlugins') }}</p>
    </div>
...
async function handleScan() {
  try {
    ...
    if (result) {
      alert(t('admin.plugins.scanSuccess', { count: result.registered_count }))
    }
  } catch (err) {
    console.error('Scan failed:', err)
    alert(t('admin.plugins.errors.scanFailed'))
  }
}
...
async function handleApprove(pluginId: string) {
  try {
    ...
    alert(t('admin.plugins.approveSuccess'))
  } catch (err) {
    console.error('Approval failed:', err)
    alert(t('admin.plugins.errors.approveFailed'))
  }
}
...
async function handleDeactivate(pluginId: string) {
  ...
  try {
    ...
    alert(t('admin.plugins.deactivateSuccess'))
  } catch (err) {
    console.error('Deactivation failed:', err)
    alert(t('admin.plugins.errors.deactivateFailed'))
  }
}
...
async function handleModalReject(pluginId: string, reason: string) {
  try {
    ...
    alert(t('admin.plugins.rejectSuccess'))
    selectedPlugin.value = null
  } catch (err) {
    console.error('Rejection failed:', err)
    alert(t('admin.plugins.errors.rejectFailed'))
  }
}
...
async function handleModalActivate(pluginId: string) {
  try {
    ...
    alert(t('admin.plugins.activateSuccess'))
    selectedPlugin.value = null
  } catch (err) {
    console.error('Activation failed:', err)
    alert(t('admin.plugins.errors.activateFailed'))
  }
}
