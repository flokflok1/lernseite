...
<template>
  <div class="lesson-preview-panel">
    ...
    <div class="preview-header">
      <div class="lesson-badge">Lektion {{ lessonPosition }}</div>
      <h1 class="lesson-title">{{ lessonData?.title || 'Lädt...' }}</h1>
      ...
      <div class="preview-content">
        ...
        <div v-if="activeTab === 'theory'" class="tab-content">
          ...
          <div v-else class="empty-state">
            ...
            <p>{{ $t('panel.lessons.noTheory') }}</p>
            <p class="empty-hint">{{ $t('panel.lessons.theoryCanBeGenerated') }}</p>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'exercises'" class="tab-content">
          ...
          <div v-else class="empty-state">
            <span class="empty-icon">📭</span>
            <p>{{ $t('panel.lessons.noMethods') }}</p>
            <p class="empty-hint">{{ $t('panel.lessons.methodsAreCreatedInStudio') }}</p>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'info'" class="tab-content">
          <div class="info-grid">
            <div class="info-card">
              <span class="info-label">{{ $t('panel.lessons.lesson') }}</span>
...
// Tabs
const tabs = computed(() => [
  { id: 'exercises', label: t('panel.lessons.exercises'), icon: '📝', count: methods.value.length },
  { id: 'theory', label: t('panel.lessons.theory'), icon: '📖' },
...
async function loadLessonData() {
  ...
  try {
    ...
    error.value = err.response?.data?.error || 'Lektion konnte nicht geladen werden'
  } finally {
    loading.value = false
  }
}
    ...
    setup(props: any) {
      ...
      if (type === 12) {
        ...
        return () => h('div', { class: 'method-preview' }, [
          items.length > 0 ? [
            h('h4', `${items.length} Schritte`),
            h('div', { class: 'exercises-preview' },
              items.slice(0, 3).map((step: any, i: number) =>
                h('div', { class: 'exercise-item' }, [
                  h('span', { class: 'exercise-label' }, `Schritt ${step.id || i + 1}:`),
                  h('span', { class: 'exercise-text' },
                    step.prompt?.substring(0, 150) + (step.prompt?.length > 150 ? '...' : '') ||
                    step.question || step.text || 'Aufgabe'
                  )
                ])
              )
            ),
            items.length > 3 ?
              h('p', { class: 'more-hint' }, `+ ${items.length - 3} weitere Schritte`) : null
          ] : h('p', { class: 'no-content' }, 'Keine Schritte konfiguriert')
        ])
      }
      ...
      if (type === 22) {
        ...
        return () => h('div', { class: 'method-preview' }, [
          questions.length > 0 ? [
            ...
            questions.length > 3 ?
              h('p', { class: 'more-hint' }, `+ ${questions.length - 3} weitere Fragen`) : null
          ] : h('p', { class: 'no-content' }, 'Keine Fragen konfiguriert')
        ])
      }
      ...
      if (type === 13) {
        ...
        return () => h('div', { class: 'method-preview' }, [
          cards.length > 0 ? [
            h('h4', `${cards.length} Karteikarten`),
            h('div', { class: 'cards-preview' },
              cards.slice(0, 3).map((card: any, i: number) =>
                h('div', { class: 'card-item' }, [
                  h('div', { class: 'card-front' }, ['Vorne: ', (card.front || card.question || `Karte ${i + 1}`)?.substring(0, 80)]),
                  h('div', { class: 'card-back' }, ['Hinten: ', (card.back || card.answer || '')?.substring(0, 100) + '...'])
                ])
              )
            ),
            cards.length > 3 ?
              h('p', { class: 'more-hint' }, `+ ${cards.length - 3} weitere Karten`) : null
          ] : h('p', { class: 'no-content' }, 'Keine Karteikarten konfiguriert')
        ])
      }
      ...
      return () => h('div', { class: 'method-preview' }, [
        itemCount > 0 ? [
          h('h4', `${itemCount} Elemente`),
          h('p', { class: 'data-hint' }, `Datenfelder: ${dataKeys.join(', ')}`),
          h('details', { class: 'config-details' }, [
            h('summary', 'JSON-Daten anzeigen'),
            h('pre', { class: 'config-json' }, JSON.stringify(config, null, 2).substring(0, 1000))
          ])
        ] : dataKeys.length > 0 && !Array.isArray(config) ? [
          h('h4', 'Konfiguration'),
          h('p', { class: 'data-hint' }, `Felder: ${dataKeys.join(', ')}`),
          h('details', { class: 'config-details' }, [
            h('summary', 'JSON-Daten anzeigen'),
            h('pre', { class: 'config-json' }, JSON.stringify(config, null, 2).substring(0, 1000))
          ])
        ] : h('p', { class: 'no-content' }, 'Keine Konfiguration')
      ])
    }
  }
}
