...
import { AudioRecorder } from '@/components/base/learning/audio'
...
const startExam = async () => {
  ...
  try {
    ...
    const response = await tutorChat({
      message: t('lesson.oral.prompts.startExam', { topic: questionTopic }),
      systemPrompt: examinerSystemPrompt.value,
      context: `${t('lesson.oral.examFormat')}: ${methodData.value.format}`
    })
...
const handleRecording = async (blob: Blob, duration: number) => {
  ...
  try {
    ...
    const examinerResponse = await tutorChat({
      message: t('lesson.oral.prompts.analyzeAnswer', {
        answer: result.transcription,
        score: result.analysis.score,
        coveredPoints: result.analysis.covered_points.join(', ') || t('lesson.oral.none'),
        missingPoints: result.analysis.missing_points.join(', ') || t('lesson.oral.none')
      }),
      systemPrompt: examinerSystemPrompt.value,
...
const handleTextAnswer = async () => {
  ...
  try {
    ...
    const examinerResponse = await tutorChat({
      message: t('lesson.oral.prompts.evaluateAnswer', { answer: userAnswer.value }),
      systemPrompt: examinerSystemPrompt.value,
...
const completeExam = async () => {
  ...
  try {
    const summaryResponse = await tutorChat({
      message: t('lesson.oral.prompts.finalSummary'),
      systemPrompt: examinerSystemPrompt.value,
