...
export function useCourseDetail(courseId: string) {
  ...
  const statusText = computed(() => {
    if (!course.value) return ''
    const statusMap = {
      draft: t('panel.courseDetail.status.draft'),
      published: t('panel.courseDetail.status.published'),
      archived: t('panel.courseDetail.status.archived')
    }
  ...
  async function loadCourse() {
    ...
    try {
      if (!courseId || courseId === 'undefined' || courseId === 'null') {
        throw new Error(`${t('panel.courseDetail.invalidCourseId')}: ${courseId}`)
      }

      course.value = await adminGetCourseDetail(courseId)
    } catch (err: any) {
      console.error('Error loading course:', err)
      error.value = err.response?.data?.message || err.message || t('panel.courseDetail.loadErrorDetails')
    } finally {
      loading.value = false
    }
  }
  ...
  async function deleteChapter(chapterToDelete: Chapter): Promise<void> {
    ...
    if (!confirm(t('panel.courseDetail.confirmDeleteChapter', { title: chapterToDelete.title }))) {
      return
    }
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.deleteChapterError'))
    }
  }
  ...
  async function publishCourse(): Promise<void> {
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.publishError'))
    }
  }
  ...
  async function unpublishCourse(): Promise<void> {
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.unpublishError'))
    }
  }
  ...
  async function archiveCourse(): Promise<void> {
    ...
    if (!confirm(t('panel.courseDetail.confirmArchive'))) {
      return
    }
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.archiveError'))
    }
  }
  ...
  async function loadCourseFiles(): Promise<void> {
    ...
    try {
      ...
      filesError.value = err.response?.data?.message || t('panel.courseDetail.filesLoadError')
    } finally {
  ...
  async function uploadFile(file: File, category: string): Promise<void> {
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.uploadError'))
    } finally {
  ...
  async function deleteFile(file: CourseFile): Promise<void> {
    ...
    if (!confirm(t('panel.courseDetail.confirmDeleteFile', { name: file.original_filename }))) {
      return
    }
    ...
    try {
      ...
      alert(err.response?.data?.message || t('panel.courseDetail.deleteFileError'))
    }
  }
