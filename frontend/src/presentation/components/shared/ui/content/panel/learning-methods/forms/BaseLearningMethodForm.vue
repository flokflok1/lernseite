  ...
  <div class="learning-method-form h-full flex flex-col bg-[var(--color-bg)]">
      ...
      <div class="space-y-6 max-w-2xl">
        ...
        <div>
          <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
            {{ $t('panel.learningMethods.form.titleLabel') }}
          </label>
          <input
            v-model="form.title"
            type="text"
            :placeholder="$t('panel.learningMethods.form.titlePlaceholder')"
        ...
        <div>
          <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
            {{ $t('panel.learningMethods.form.instructionsLabel') }}
          </label>
          <textarea
            v-model="form.instructions"
            rows="3"
            :placeholder="$t('panel.learningMethods.form.instructionsPlaceholder')"
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
              {{ $t('panel.learningMethods.form.durationLabel') }}
            </label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
              {{ $t('panel.learningMethods.form.difficultyLabel') }}
            </label>
            <select
              v-model="form.difficulty"
              class="w-full px-3 py-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
            >
              <option value="easy">{{ $t('panel.learningMethods.form.difficultyEasy') }}</option>
              <option value="medium">{{ $t('panel.learningMethods.form.difficultyMedium') }}</option>
              <option value="hard">{{ $t('panel.learningMethods.form.difficultyHard') }}</option>
            </select>
          </div>
        </div>
      </div>
    ...
    <div class="bg-[var(--color-surface)] border-t border-[var(--color-border)] px-4 py-3">
      <div class="flex gap-3 justify-end">
        ...
        >
          {{ $t('panel.learningMethods.form.cancelButton') }}
        </button>
        <button
          @click="save"
        ...
        >
          {{ isSaving ? $t('panel.learningMethods.form.savingButton') : (isEditMode ? $t('panel.learningMethods.form.updateButton') : $t('panel.learningMethods.form.createButton')) }}
        </button>
      </div>
    </div>
  </div>
...
const save = async () => {
  if (!form.value.title.trim()) {
    alert(t('panel.learningMethods.form.emptyTitleError'))
    return
  }

  if (!chapterId.value) {
    alert(t('panel.learningMethods.form.noChapterIdError'))
    return
  }
  ...
  try {
    ...
    alert(error.response?.data?.message || t('panel.learningMethods.form.saveErrorMessage'))
  } finally {
