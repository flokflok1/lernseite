<template>
  <form @submit.prevent="handleSubmit" class="role-form space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            type="text"
            required
            :placeholder="$t('panel.groups.form.displayNamePlaceholder')"
            minlength="3"
            maxlength="100"
            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
          />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ $t('panel.groups.form.displayNameHint') }}
          </p>
        </div>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {{ $t('panel.groups.form.hierarchyLevelLabel') }} *
          </label>
          <input
          ...
          />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ $t('panel.groups.form.hierarchyLevelHint') }}
          </p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            type="text"
            maxlength="10"
            :placeholder="$t('panel.groups.form.iconPlaceholder')"
            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
          />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ $t('panel.groups.form.iconHint') }}
          </p>
        </div>
        ...
        <div class="md:col-span-2">
          ...
          ></textarea>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ $t('panel.groups.form.descriptionHint') }}
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
        {{ $t('panel.groups.form.selectFeatures') }}
      </h3>
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        {{ $t('panel.groups.form.featuresHint') }}
      </p>
      ...
      <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
        {{ formData.feature_ids.length }} {{ $t('panel.groups.features') }}
        {{ $t('panel.groups.features.assigned') }}
      </div>
    </div>
    ...
    >
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
        {{ $t('panel.groups.form.selectPermissions') }}
      </h3>
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        {{ $t('panel.groups.form.permissionsHint') }}
      </p>
      ...
      <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
        {{ formData.permission_ids.length }} {{ $t('panel.groups.permissions') }}
        {{ $t('panel.groups.permissions.assigned') }}
      </div>
    </div>
        ...
        <div class="flex items-center space-x-3">
          ...
          <div>
            <div class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ formData.display_name || $t('panel.groups.form.displayNamePlaceholder') }}
            </div>
            <div class="text-sm text-gray-500 dark:text-gray-400">
              {{ formData.role_name || $t('panel.groups.form.roleNamePlaceholder') }}
            </div>
          </div>
        </div>
    ...
    <div class="flex justify-end gap-3">
      ...
      >
        {{ submitting ? $t('common.saving') : (isEditMode ? $t('panel.groups.form.update') : $t('panel.groups.form.create')) }}
      </button>
    </div>
  </form>
</template>
