  ...
  <div class="permission-matrix">
    ...
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">
        {{ mode === 'features' ? $t('panel.groups.features.title') : $t('panel.groups.permissions.title') }}
      </h2>
      <p class="text-gray-600 dark:text-gray-400">
        {{ mode === 'features' ? $t('panel.groups.features.subtitle') : $t('panel.groups.permissions.subtitle') }}
      </p>
      <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
        {{ $t('panel.groups.roleName') }}: <span class="font-semibold">{{ role.display_name }}</span>
      </div>
    </div>
    ...
    <div class="tabs flex gap-2 mb-6">
      ...
      >
        🔐 {{ $t('panel.groups.permissions.title') }}
        <span class="ml-2 px-2 py-1 rounded bg-white bg-opacity-20 text-xs">
      ...
      <label class="flex items-center justify-between cursor-pointer">
        <div>
          <div class="font-medium text-gray-900 dark:text-white">
            {{ replaceMode ? $t('panel.groups.form.replaceMode') : $t('panel.groups.form.addMode') }}
          </div>
          <div class="text-sm text-gray-500 dark:text-gray-400">
            {{ replaceMode ? $t('panel.groups.form.replaceModeHint') : $t('panel.groups.form.addModeHint') }}
          </div>
        </div>
        <div class="relative">
          <input
      ...
      >
        ...
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ $t(`panel.groups.features.categories.${category}`) }}
          </h3>
          <div class="flex gap-2">
            ...
            >
              {{ $t('panel.groups.features.selectAll') }}
            </button>
            ...
            >
              {{ $t('panel.groups.features.deselectAll') }}
            </button>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex gap-2">
            <button
              ...
              class="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
            >
              {{ $t('panel.groups.permissions.selectAll') }}
            </button>
            <span class="text-gray-400">|</span>
            <button
              ...
              class="text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300"
            >
              {{ $t('panel.groups.permissions.deselectAll') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex justify-between items-center">
        <div class="text-sm text-gray-600 dark:text-gray-400">
          <span v-if="mode === 'features'">
            {{ selectedFeatures.length }} / {{ availableFeatures.length }} {{ $t('panel.groups.features') }}
          </span>
          <span v-else>
            {{ selectedPermissions.length }} / {{ availablePermissions.length }} {{ $t('panel.groups.permissions') }}
          </span>
        </div>
        ...
        <div class="flex gap-3">
          ...
          >
            {{ submitting ? $t('common.saving') : (mode === 'features' ? $t('panel.groups.features.assign') : $t('panel.groups.permissions.assign')) }}
          </button>
        </div>
      </div>
    </div>
