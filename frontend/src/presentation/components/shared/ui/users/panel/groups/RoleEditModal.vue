    ...
    >
      <Card class="w-full max-w-md p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
          {{ mode === 'create' ? $t('panel.groups.createRole') : $t('panel.groups.editRole') }}
        </h2>

        <div class="space-y-4">
          <div v-if="mode === 'create'">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              {{ $t('panel.groups.technicalName') }}
            </label>
            <Input v-model="form.role_name" :placeholder="$t('panel.groups.technicalNamePlaceholder')" />
          </div>
          ...
          <div>
            ...
            <Input v-model="form.display_name" :placeholder="$t('panel.groups.displayNamePlaceholder')" />
          </div>
