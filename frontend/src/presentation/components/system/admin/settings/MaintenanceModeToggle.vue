  ...
  <div class="maintenance-toggle bg-[#1a1f35] rounded-lg p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-400">
          {{ $t('admin.systemSettings.maintenanceModeDescription') }}
        </p>
      </div>
      ...
      <div class="flex items-start gap-3">
        ...
        <div>
          <p class="text-sm font-semibold text-yellow-200 mb-1">
            {{ $t('admin.systemSettings.maintenanceModeActive') }}
          </p>
          <p class="text-xs text-yellow-300">
            {{ $t('admin.systemSettings.onlyAdminsCanAccess') }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="maintenanceEnabled" class="mb-6">
      <label class="block text-sm font-medium text-gray-300 mb-2">
        {{ $t('admin.systemSettings.customMessage') }}
      </label>
      <textarea
        v-model="maintenanceMessage"
        rows="3"
        :placeholder="$t('admin.systemSettings.messagePlaceholder')"
      ...
      <p class="mt-2 text-xs text-gray-400">
        {{ $t('admin.systemSettings.messageHint') }}
      </p>
      ...
      <div class="flex gap-3">
        ...
        <div>
          <p class="text-sm font-semibold text-blue-200 mb-1">
            {{ $t('admin.systemSettings.maintenanceInfo.title') }}
          </p>
          <ul class="text-xs text-blue-300/80 space-y-1">
            <li>• {{ $t('admin.systemSettings.maintenanceInfo.point1') }}</li>
            <li>• {{ $t('admin.systemSettings.maintenanceInfo.point2') }}</li>
            <li>• {{ $t('admin.systemSettings.maintenanceInfo.point3') }}</li>
          </ul>
        </div>
      </div>
    </div>
...
const toggleMode = async () => {
  ...
  if (success) {
    ...
    successMessage.value = newState
      ? t('admin.systemSettings.maintenanceEnabled')
      : t('admin.systemSettings.maintenanceDisabled')
...
const saveMessage = async () => {
  ...
  if (success) {
    successMessage.value = t('admin.systemSettings.messageSaved')
