<template>
  ...
  <Card v-if="role" class="p-6">
    ...
    <div>
      <h3 class="font-semibold text-gray-900 dark:text-white mb-3">
        {{ $t('panel.groups.usersWithRole', { count: users.length }) }}
      </h3>
      <div class="space-y-2 max-h-48 overflow-y-auto">
        ...
        <p v-if="users.length === 0" class="text-gray-500 text-sm p-2">
          {{ $t('panel.groups.noUsersWithRole') }}
        </p>
      </div>
    </div>
  </Card>
  ...
  <Card v-else class="p-12 text-center">
    ...
    <p class="text-gray-500">{{ $t('panel.groups.selectRoleHint') }}</p>
  </Card>
</template>
...
import type { Role, Permission, RoleUser } from '@/application/services/api/admin'
