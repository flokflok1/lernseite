  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-end gap-3">
      ...
      <div class="w-40">
        ...
        >
          <option v-for="lang in nonPrimaryLanguages" :key="lang.language_code" :value="lang.language_code">
...
} from '@/infrastructure/api/clients/admin/languages.api'
...
// Known namespaces (matching backend seed)
const namespaces = [
  'aiEditor', 'common', 'courses', 'dashboard', 'errors',
  'features', 'legal', 'setup', 'tutor',
  'panel.system', 'panel.languages', 'panel.ai_settings',
  'panel.analytics', 'panel.course_management', 'panel.feature_flags',
  'panel.groups', 'panel.organisations', 'panel.shared', 'panel.users'
]

const nonPrimaryLanguages = computed(() =>
  props.languages.filter(l => !l.is_primary)
)
...
watch(() => props.languages, (langs) => {
  if (langs.length > 0 && !filters.value.language) {
    ...
    if (nonPrimary.length > 0) {
      filters.value.language = nonPrimary[0].language_code
      loadReview(1)
    }
  }
}, { immediate: true })
