...
const loadProviders = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/v1/panel/ai/providers?include_inactive=true', {
    ...
    if (axios.isAxiosError(err)) {
      error.value = err.response?.data?.error || t('panel.aiSettingsPage.messages.networkError')
    } else {
...
const loadModels = async () => {
  try {
    ...
    const response = await axios.get('/api/v1/panel/ai/models/grouped', {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const loadSettings = async () => {
  try {
    const token = localStorage.getItem('access_token')
    const response = await axios.get('/api/v1/panel/ai/settings', {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const saveApiKey = async (provider: AIProvider) => {
  ...
  try {
    const token = localStorage.getItem('access_token')
    const response = await axios.put(
      `/api/v1/panel/ai/providers/${provider.provider_id}/api-key`,
      { api_key: key },
      { headers: { Authorization: `Bearer ${token}` } }
    )
...
const testApiKey = async (provider: AIProvider) => {
  ...
  try {
    ...
    const response = await axios.post(
      `/api/v1/panel/ai/providers/${provider.provider_id}/test`,
      {},
      { headers: { Authorization: `Bearer ${token}` } }
    )
...
const toggleActive = async (provider: AIProvider) => {
  try {
    ...
    const response = await axios.patch(
      `/api/v1/panel/ai/providers/${provider.provider_id}`,
