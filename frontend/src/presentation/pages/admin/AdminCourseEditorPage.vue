<template>
  <div class="admin-course-editor-page py-8">
    <!-- Header Section -->
    <div class="max-w-4xl mx-auto mb-12">
      <div class="text-center">
        <div class="text-5xl mb-4">📝</div>
        <h1 class="text-4xl font-bold text-gray-900 mb-2">{{
          $t('admin.courseEditor.pageTitle')
        }}</h1>
        <p class="text-lg text-gray-600">{{
          $t('admin.courseEditor.pageSubtitle')
        }}</p>
      </div>
    </div>
        ...
        <div class="flex items-center justify-center gap-3">
          ...
          <div class="text-left">
            <div class="text-xl font-bold">{{
              $t('admin.courseEditor.openCourseEditor')
            }}</div>
            <div class="text-sm opacity-90">{{
              $t('admin.courseEditor.chooseEditorType', 'Select between AI and Manual editing')
            }}</div>
          </div>
        </div>
      </button>
    ...
    <div class="max-w-4xl mx-auto mb-12">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">{{
        $t('admin.courseEditor.features', 'Features')
      }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="p-6 bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-200 rounded-lg hover:shadow-lg transition-shadow">
          ...
          <h3 class="text-xl font-bold text-gray-900 mb-2">{{
            $t('admin.courseEditor.aiEditorFeature', 'AI-Powered Editor')
          }}</h3>
          <ul class="text-sm text-gray-700 space-y-2">
            <li>✓ {{ $t('admin.courseEditor.feature1', 'Upload documents (PDF, DOCX, PPTX, etc.)') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature2', 'Auto-generate course structure') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature3', 'AI-powered content generation') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature4', 'Smart learning method suggestions') }}</li>
          </ul>
        </div>
        ...
        <div class="p-6 bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-200 rounded-lg hover:shadow-lg transition-shadow">
          ...
          <h3 class="text-xl font-bold text-gray-900 mb-2">{{
            $t('admin.courseEditor.manualEditorFeature', 'Manual Editor')
          }}</h3>
          <ul class="text-sm text-gray-700 space-y-2">
            <li>✓ {{ $t('admin.courseEditor.feature5', 'Full control over structure') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature6', 'Detailed chapter management') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature7', 'Granular lesson editing') }}</li>
            <li>✓ {{ $t('admin.courseEditor.feature8', 'Direct content input') }}</li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <div v-if="recentCourses.length > 0" class="max-w-4xl mx-auto mb-12">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">{{
        $t('admin.courseEditor.recentCourses')
      }}</h2>
    ...
    <div class="max-w-4xl mx-auto">
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h3 class="font-bold text-blue-900 mb-3">💡 {{
          $t('admin.courseEditor.tips', 'Tips')
        }}</h3>
        <ul class="text-sm text-blue-800 space-y-2">
          <li>• {{ $t('admin.courseEditor.tip1', 'Use AI Editor to quickly structure courses from documents') }}</li>
          <li>• {{ $t('admin.courseEditor.tip2', 'Use Manual Editor for fine-grained control') }}</li>
          <li>• {{ $t('admin.courseEditor.tip3', 'You can open multiple editors at the same time') }}</li>
          <li>• {{ $t('admin.courseEditor.tip4', 'All changes are auto-saved to your workspace') }}</li>
        </ul>
      </div>
    </div>
