<template>
  <div class="admin-translations-page p-6">
    <!-- Header -->
    <div class="mb-6 flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-[var(--color-text-primary)]">{{ $t('admin.translations.title') }}</h1>
        <p class="text-sm text-[var(--color-text-secondary)]">
          {{ languages.length }} {{ $t('admin.translations.languages') }} · {{ totalKeys }} Keys
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-else class="flex items-center gap-2 group cursor-pointer" @click="startEdit(item)">
                    <span
                      ...
                      :title="item.translated_value || $t('admin.translations.notTranslated')"
                    >
                      {{ item.translated_value || $t('admin.translations.notTranslated') }}
                    </span>
    ...
    <div v-if="showAddLanguageModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showAddLanguageModal = false">
      <div class="bg-[var(--color-surface)] rounded-lg p-6 w-full max-w-md">
        <h3 class="text-lg font-semibold text-[var(--color-text-primary)] mb-4">{{ $t('admin.translations.addLanguageTitle') }}</h3>

        <div class="space-y-4">
          <div>
            <label class="block text-sm text-[var(--color-text-secondary)] mb-1">{{ $t('admin.translations.languageCode') }}</label>
          ...
          <div>
            <label class="block text-sm text-[var(--color-text-secondary)] mb-1">{{ $t('admin.translations.languageName') }}</label>
          ...
          <div>
            <label class="block text-sm text-[var(--color-text-secondary)] mb-1">{{ $t('admin.translations.nativeName') }}</label>
          ...
          <div>
            <label class="block text-sm text-[var(--color-text-secondary)] mb-1">{{ $t('admin.translations.flag') }}</label>
...
import deCommon from '@/infrastructure/i18n/de/common.json'
import deAdmin from '@/infrastructure/i18n/de/admin.json'
import deWindowsAdmin from '@/infrastructure/i18n/de/windows/admin.json'
import deWindowsAiStudio from '@/infrastructure/i18n/de/windows/aiStudio.json'
import deWindowsCommon from '@/infrastructure/i18n/de/windows/common.json'
import deWindowsViewer from '@/infrastructure/i18n/de/windows/viewer.json'
import deWindowsLearningMethods from '@/infrastructure/i18n/de/windows/learningMethods.json'
import deCourses from '@/infrastructure/i18n/de/courses.json'
import deDashboard from '@/infrastructure/i18n/de/dashboard.json'
import deSetup from '@/infrastructure/i18n/de/setup.json'
import deLegal from '@/infrastructure/i18n/de/legal.json'

import enCommon from '@/infrastructure/i18n/en/common.json'
import enAdmin from '@/infrastructure/i18n/en/admin.json'
import enWindowsAdmin from '@/infrastructure/i18n/en/windows/admin.json'
import enWindowsAiStudio from '@/infrastructure/i18n/en/windows/aiStudio.json'
import enWindowsCommon from '@/infrastructure/i18n/en/windows/common.json'
import enWindowsViewer from '@/infrastructure/i18n/en/windows/viewer.json'
import enWindowsLearningMethods from '@/infrastructure/i18n/en/windows/learningMethods.json'
import enCourses from '@/infrastructure/i18n/en/courses.json'
import enDashboard from '@/infrastructure/i18n/en/dashboard.json'
import enSetup from '@/infrastructure/i18n/en/setup.json'
import enLegal from '@/infrastructure/i18n/en/legal.json'

import plCommon from '@/infrastructure/i18n/pl/common.json'
import plAdmin from '@/infrastructure/i18n/pl/admin.json'
import plWindowsAdmin from '@/infrastructure/i18n/pl/windows/admin.json'
import plWindowsAiStudio from '@/infrastructure/i18n/pl/windows/aiStudio.json'
import plWindowsCommon from '@/infrastructure/i18n/pl/windows/common.json'
import plWindowsViewer from '@/infrastructure/i18n/pl/windows/viewer.json'
import plWindowsLearningMethods from '@/infrastructure/i18n/pl/windows/learningMethods.json'
import plCourses from '@/infrastructure/i18n/pl/courses.json'
import plDashboard from '@/infrastructure/i18n/pl/dashboard.json'
import plSetup from '@/infrastructure/i18n/pl/setup.json'
import plLegal from '@/infrastructure/i18n/pl/legal.json'
...
async function loadTranslations() {
  ...
  try {
    const response = await http.get('/i18n/admin/keys', { params: { limit: 1000 } })
...
async function saveTranslation(item: TranslationItem) {
  try {
    ...
    statusMessage.value = { type: 'success', text: t('admin.translations.saved') }
    setTimeout(() => statusMessage.value = null, 2000)
  } catch (error: any) {
    statusMessage.value = { type: 'error', text: t('admin.translations.saveFailed') }
  } finally {
    cancelEdit()
  }
}
...
async function addLanguage() {
  ...
  try {
    await http.post('/i18n/admin/languages', {
    ...
    statusMessage.value = { type: 'success', text: t('admin.translations.languageAdded') }
