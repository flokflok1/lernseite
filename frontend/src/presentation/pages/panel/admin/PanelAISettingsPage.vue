              ...
              <div>
                ...
                <div class="flex gap-2">
                  ...
                  <button
                    ...
                    :title="showApiKey[provider.provider_id] ? $t('panel.aiSettingsPage.apiKey.hide') : $t('panel.aiSettingsPage.apiKey.show')"
                  >
                ...
                <div class="flex gap-2 mt-3">
                  ...
                  >
                    ...
                    <span>{{ savingKey[provider.provider_id] ? $t('panel.aiSettingsPage.apiKey.saving') : $t('panel.aiSettingsPage.apiKey.save') }}</span>
                  </button>
                  ...
                  >
                    ...
                    <span>{{ testingKey[provider.provider_id] ? $t('panel.aiSettingsPage.apiKey.testing') : $t('panel.aiSettingsPage.apiKey.test') }}</span>
                  </button>
                  ...
                  <div class="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p v-if="testResults[provider.provider_id].response_time" class="text-xs opacity-75">
                        {{ $t('panel.aiSettingsPage.messages.responseTime', { time: testResults[provider.provider_id].response_time }) }}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div class="space-y-4">
                  ...
                  <div v-if="provider.last_validated" class="text-sm text-[var(--color-text-secondary)]">
                    {{ $t('panel.aiSettingsPage.settings.lastValidated') }} {{ formatDate(provider.last_validated) }}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="p-6">
          ...
          <div class="mt-4 flex justify-end">
            ...
            >
              ...
              <span>{{ savingSettings ? $t('panel.aiSettingsPage.defaultModel.savingSettings') : $t('panel.aiSettingsPage.defaultModel.saveSettings') }}</span>
            </button>
          </div>
        ...
        <div class="px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div>
              ...
              <p class="text-sm text-[var(--color-text-secondary)]">
                {{ $t('panel.aiSettingsPage.availableModels.modelsFromProviders', { count: totalModelCount, providers: Object.keys(availableModels).length }) }}
              </p>
            </div>
            <!-- Provider Badges -->
...
const loadProviders = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/v1/panel/settings/ai/providers?include_inactive=true', {
    ...
    if (axios.isAxiosError(err)) {
      ...
      error.value = typeof errData === 'string' ? errData : (errData?.code || t('panel.aiSettingsPage.messages.networkError'))
    } else {
      error.value = t('panel.aiSettingsPage.messages.unknownError')
    }
  } finally {
    loading.value = false
  }
...
const loadModels = async () => {
  try {
    const token = localStorage.getItem('access_token')
    const response = await axios.get('/api/v1/panel/settings/ai/models?include_inactive=true', {
      headers: { Authorization: `Bearer ${token}` }
    })

    if (response.data.success) {
      const models = response.data.data.models || []
      // Group models by provider name for the UI
...
const loadSettings = async () => {
  try {
    const token = localStorage.getItem('access_token')
    // Try loading default model for 'chat' category
    const response = await axios.get('/api/v1/panel/settings/ai/models/default/chat', {
      headers: { Authorization: `Bearer ${token}` }
    })
    ...
    // Endpoint may not return data yet — use defaults
...
const saveDefaultSettings = async () => {
  ...
  try {
    const token = localStorage.getItem('access_token')
    // Find the model_id for the selected model to set it as default
    ...
    if (!selectedModel?.model_id) {
      settingsResult.value = { success: false, message: t('panel.aiSettingsPage.messages.modelNotFound') }
      savingSettings.value = false
      return
    }
    // Use the set-default endpoint for the selected model
    ...
    settingsResult.value = {
      ...
      message: axios.isAxiosError(err)
        ? (err.response?.data?.error || t('panel.aiSettingsPage.messages.networkErrorGeneric'))
        : t('panel.aiSettingsPage.messages.unknownErrorGeneric')
    }
  } finally {
    savingSettings.value = false
  }
...
const testApiKey = async (provider: AIProvider) => {
  ...
  try {
    ...
    testResults.value[provider.provider_id] = {
      success: false,
      message: axios.isAxiosError(err)
        ? (err.response?.data?.error || t('panel.aiSettingsPage.messages.testNetworkError'))
        : t('panel.aiSettingsPage.messages.unknownErrorGeneric')
    }
  } finally {
...
const deleteApiKey = async (provider: AIProvider) => {
  ...
  try {
    ...
    testResults.value[provider.provider_id] = {
      success: false,
      message: axios.isAxiosError(err)
        ? (err.response?.data?.error || t('panel.aiSettingsPage.messages.removeError'))
        : t('panel.aiSettingsPage.messages.unknownErrorGeneric')
    }
  } finally {
